...
- The five `Spike` tickets remain research-only and executable within their exhaustive allowlists after their declared dependencies merge. `AST-H001`, `PATH-H002`, `ASSET-I001`, and `PKG-M001` require merged `CI-M003` because their evidence uses the managed role workflows. A draft or unmerged branch never satisfies that dependency.
...
Each tranche uses its own isolated worktree, branch, and pull request created from the then-current merged `master`. Each ticket is one milestone and receives one full milestone review before the next ticket starts. Merge every prerequisite pull request before creating the dependent tranche; no unmerged branch or draft pull request satisfies a dependency.
