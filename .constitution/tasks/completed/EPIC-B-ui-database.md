...
**Post-PR-review round 4 (dual-axis review on PR #4) — confirming pass after round 3's material behavior changes, no fixes needed.** Round 3's own fixes (search semantics, key zeroization, widget split, schema version) had never themselves been reviewed, so a fourth pass ran against the actual current code rather than assuming rounds 2–3's clean verdicts covered it. Both axes returned zero P0/P1 — three consecutive clean high-priority rounds (2, 3, 4) — so the loop stops here per its own two-consecutive-clean-round rule; the following are recorded as deferred, not fixed, consistent with "don't invent scope beyond what's grounded in the constitution":
- **Feature Envy left half-resolved (P2, Standards):** round 2's Divergent-Change fix extracted the draft-state domain out of `api::ffi_api` but left `search_notes_impl`/`save_note_impl`/`fts5_phrase_query` in place, even though they operate exclusively on `db`-owned data (`containers.md` assigns full-text queries to the Local Repository container, and `guidelines.md` describes `api/` as "thin `#[frb]` wrappers"). Moving this logic into `db` is mechanical and low-risk, but doing so now — three clean rounds deep — would mean re-reviewing a purely organizational change with no correctness or user-facing benefit, which the loop's own diminishing-returns guidance says to stop chasing. Left as a known cleanup for whoever next touches `db`/`api`.
- **No error surface for FFI failures in `note_providers.dart` (P2, Spec):** `NoteController.updateBlock` doesn't handle an `AppError` the Rust side could raise (e.g. a bad `block_path`). Unreachable today — `Editor` isn't mounted in `main.dart` and only ever emits valid top-level paths — so there is nothing to exercise yet; revisit once the Editor is actually wired into the app.
- **Search is not scoped to a workspace (Investigate, Spec):** `search_notes_impl` has no `workspace_id` predicate, while `prd/capabilities.md`'s P0 capability reads "search across all Notes **in their Workspace**." Currently unreachable — `open_note` hardcodes a single `"default"` workspace and no workspace-selection ticket has ever landed — but the schema (`workspaces`, `notes.workspace_id`, `UNIQUE(workspace_id, path)`) is already multi-workspace-shaped. Adding a filter now would mean inventing the workspace-selection mechanism this epic never scoped; deferred to whichever future ticket introduces multi-workspace support.
- **General feedback (Standards):** a test asserting the two `schema.sql` copies stay byte-identical (currently just eyeballed via `diff` each round) would catch drift automatically; `_paragraphStyle` and `renderInline`'s `TextRun`→`TextStyle` mapping could share a helper; a `TextRun.plain(text)` factory would DRY the recurring plain-run construction in `editor.dart` and its tests. All minor polish, not acted on.
