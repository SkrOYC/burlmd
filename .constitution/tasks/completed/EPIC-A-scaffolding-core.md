...
- **Touched Files:** `analysis_options.yaml`, `devenv.nix`, `lib/`, `linux/`, `macos/`, `rust/src/api/simple.rs`, `rust_builder/`, `integration_test/`, `test/`, `test_driver/`, `.metadata`, `pubspec.lock`
- **Justification:** Standard Flutter project initialization (`flutter create`) and `flutter_rust_bridge` monorepo integration (`flutter_rust_bridge_codegen integrate`) scaffolded platform build trees, cargokit wrapper package, default main entrypoint, and analysis exclusions required for Dart and Rust build support. `rust/src/api/simple.rs` carries the FRB `#[frb(init)]` lifecycle hook (`init_app`) generated by the same scaffolding. `devenv.nix` was updated to add `integration_test/`/`test_driver/` to the `dart format` scope once `flutter_rust_bridge_codegen integrate` created those directories.
