...
##### WSPC-D002 Deviations & Justifications
- **Touched Files:** `rust/Cargo.lock`, `.constitution/tech-spec/stack.md`
...
##### WSPC-D006 Deviations & Justifications
- **Touched Files:** `rust/src/workspace/persist.rs` (additive only)
- **Justification:** `contracts/ffi_api.rs` states three obligations on `rename_note` — the working source, the span map and the recorded revision of *every affected Note* must be carried forward — and all three are discharged against state that `workspace::persist` keeps private. `SessionState`'s fields live behind a private `Mutex`, and a `SessionInner`'s identity (its concept id and the two paths derived from it) is immutable by construction precisely because every reader of it reads without a lock: the idle timer, tier 2's OCC sequence and tier 3's pathspec commit. There is therefore no way to express a session re-key from `workspace/lifecycle.rs`, and refusing to rename an open Note — the alternative the contract itself names — is explicitly not the answer taken, because `SHEL-E005` makes renaming an open Note a criterion-backed workflow and the set of affected *source* Notes is not something the user can see in order to close them first. The change is additive: two new `pub(super)` functions (`carry_session_forward`, `discard_session`) and one private method (`NoteSession::install_rewrite`), plus three visibility widenings from private to `pub(super)` (`Workspace::with_db`, `Workspace::note_path`, `atomic_write`). No existing signature, body or behavior changed, and `pub(super)` keeps all of it private to the `workspace` module. Re-implementing `atomic_write` inside `lifecycle.rs` was the way to touch nothing, and was rejected because `architecture/resilience.md`'s survive-an-abrupt-termination guarantee would then have two implementations free to drift.
- **In-scope amendment worth recording:** `the_ffi_surface_has_one_concept_id_open_entry_point_and_no_function_takes_a_workspace_id` in `rust/src/api/ffi_api.rs` (an in-scope file, but the assertion is `WSPC-D008`'s) now admits `create_note` as a third function returning `Result<NoteState, AppError>` while naming neither a `block_path` nor a `range`. This is an instance of the rule rather than an exception to it: the contract specifies that `create_note` creates the file **and opens it**, and it is still not a second way to open an *existing* Note, since it returns `PathUnavailable` when the path is taken. The assertion's message names all three and still fails on a fourth.
- **Four interpretation decisions, recorded because the contract leaves each implicit:** (1) An inbound Link's **display text is preserved byte for byte** and only its destination is rewritten — the text is the user's prose under `prd/constraints.md`'s Edit Fidelity constraint, and re-serializing it would double-escape a title an author escaped by hand. (2) `LifecycleEffects.remapped` and `.rewritten` both **exclude the Note the operation was invoked on**, per the struct's own docstring ("what a lifecycle operation changed, *beyond* the Note it was invoked on"); `rename_note` and `move_note` therefore return an empty `remapped`, since the returned `NoteState` already carries the new id. (3) `delete_directory` is **recursive**, per "deletes a Directory and everything beneath it, in one commit" — not empty-only; what makes that safe is the commit, which leaves every removed Note recoverable exactly as a single `delete_note` does. (4) `move_note` **does** rewrite a self-Link, even though the contract says a move changes the Note's id "but not its bytes": that sentence is about the frontmatter title, and a self-Link is an inbound Link like any other, so leaving it would point it at the id the move just vacated — risk 8's corruption arriving through the one Note the operation was invoked on.
- **One case refused rather than rewritten:** a reference-style inbound Link (`[a][ref]`, whose destination lives in a definition elsewhere) fails the whole operation with `ParseError` instead of being skipped. Skipping it is the silent partial rewrite the first STOP condition forbids; refusing keeps "atomic or fail" true. burlmd never writes this form, so it can only arise in a bundle another tool authored.
- **Review round 2 widened the same `persist.rs` deviation, still additively,** by two further `pub(super)` functions. `open_note_ids` exists because the inbound-Link sweep cannot be driven by the `links` table alone: that table only knows about Links that reached disk **and** were indexed, so a Link living only in an open session's working source, or only in an unflushed `drafts` row, is invisible to it — and the Note's own next tier 2 write then puts the dead concept id on disk and indexes it, recreating the ghost edge the rename removed and handing CAP-GRAPH-04's create-on-follow the concept back. The sweep now also seeds candidates from `drafts`, which needs no new surface. `with_write_locks` exists because a lifecycle rewrite is a tier 2 writer with a longer read-to-write gap than the idle timer's own: a timer firing inside that gap writes its buffer and the lifecycle operation then overwrites it from the snapshot it took beforehand, losing the user's work from the file *and* the buffer at once. It takes every open session's tier 2 write lock in sorted concept-id order (deterministic, so two concurrent lifecycle operations cannot deadlock against each other), preserving the module's documented write → state → connection order. Both are `pub(super)`; no existing behaviour changed.

  Also in that round, and inside this ticket's own files: `FileJournal` gained a `Drop` that discards trash entries, because `commit_paths` can fail *after* the bundle and the index have both settled and the journal was then dropped with a live `Trashed` step — parking a `.burlmd-trash.*` entry holding a deleted Note's full content as an untracked file inside the bundle, which a later broad commit or a sync could publish. `delete_directory`'s pathspec became the Directory itself rather than the list of `notes` rows removed, because the trash step carries away everything beneath it and a bundle legitimately holds files that are not Notes — an attachment, a foreign tool's `index.md` — each of which was otherwise left as a deletion Git knew about but was never told to record.
...
- **`split_block`'s `offset` is a character offset into the Block's source, and every offset inside the Core stays in bytes.** `contracts/ffi_api.rs` specifies "a character offset ... into the Block's **source** text", the Dart wrapper documents the same thing, and `WSPC-D008`'s implementation indexed bytes (`span.start + offset`) — so `Café x` split at offset 5 broke before the space instead of before the `x`, and every character past the first multibyte one in a Block drifted. Settled toward the contract's reading in PR #7 review round 11: the FFI boundary converts the caller's character offset against the Block's own source, and `SpanMap`, the splice ranges and `markdown::spans` continue to record and index **bytes**, which is what `String::replace_range` needs. No contract signature changed and no other entry point takes an offset in the Block's source, so this is the only conversion point.

Neither of the first two files is in any ticket's Scope list, and neither should have been: both were made stale *by* the epic rather than needed *for* it, which is precisely the class of change a closeout reconciliation exists to absorb.
...
**Validation.** 312 Rust tests pass (`cargo test -- --skip keyring`; 3 keyring tests filtered, as they require an OS keychain), alongside 18 Flutter widget tests and a clean `dart analyze`. Each ticket additionally carried its own Verification Command and Gherkin gate, and the measured criteria were met rather than asserted: the tier 1 draft write stays inside the frame budget at 100 KiB, a full-text query over a thousand Notes stays within budget, and the splice round-trip property holds byte-for-byte over generated Notes.

**A correction to this epic's own preamble.** It states "No ticket here touches Dart." That is wrong, and it was wrong when written. Three tickets touched hand-written Dart: `WSPC-D006` and `WSPC-D008` and `WSPC-D009` landed the wrapper and provider halves (`lib/src/providers/rust_api_provider.dart`, `lib/src/providers/note_providers.dart`) that the regenerated bindings require a call site for, and `WSPC-D008` additionally adapted `lib/src/components/editor.dart` and its test to the `save_note` → `flush_note` change. The gate applied was `dart analyze` rather than a launched application, per the accepted reading of the preamble's second claim — which *is* true: nothing in this epic is reachable in the running application, which still opens to the login screen until `SHEL-E001` removes it. A closeout smoke check confirmed that: `cargo build --release` succeeds, `flutter run -d linux` launches, and the app reaches the login screen without crashing.

The following were identified during planning, implementation or review and are explicitly out of this epic's scope. They are recorded here rather than silently dropped:

1. **An undeclared dependency edge, `WSPC-D007` → `WSPC-D006`, surfaced during planning.** `D006`'s acceptance criteria need `D007`'s pathspec-scoped commit and its write tiers — a lifecycle operation has to commit exactly the paths it touched, and it has to carry every affected Note's buffer and draft row forward. The published DAG had them independent. Corrected in the critical path at v1.5.0 and recorded there so future DAGs model it rather than rediscovering it mid-execution.
2. **A rare flake, since root-caused and fixed — and the record of it here was wrong twice over.** `git::operations::tests::path_in_head_distinguishes_a_new_note_from_one_already_in_history` failed roughly 1 run in 15 early in this branch. This note originally called it "pre-existing — it predates this epic and was not introduced by it", and that was false: `path_in_head` and its test are *added* by this epic, and `master` carries neither, so nothing about it predates anything. It was then not reproducible in 40+ subsequent runs and was provisionally recorded as environmental (parallel tempdir `git init`), which was also wrong. PR #7 review round 13 reproduced it and found the cause, which is neither the test nor the Create-vs-Update decision path it exercises: the two `apply_credentials` tests in the same module built their fixture by `std::env::set_var`ing `GIT_CONFIG_COUNT`/`GIT_CONFIG_KEY_0`/`GIT_CONFIG_VALUE_0` into the **process** environment. `git` reads that sequence out of the environment it inherits at `fork`/`exec`, three `set_var` calls are not one atomic step, and `ENV_LOCK` deliberately does not cover the many tests here that spawn a real `git` child — so any `git init` landing after `GIT_CONFIG_COUNT=1` and before `GIT_CONFIG_VALUE_0` exists died with `error: missing config value for GIT_CONFIG_VALUE_0`, failing whichever unrelated test happened to be running. `path_in_head`'s test was simply one of a dozen candidates, which is why it looked intermittent and unattributable. Fixed by passing the inherited entry count into `apply_credentials_after` instead of mutating process state, so those two tests touch nothing outside their own `Command`; the module's tests now pass repeatedly under the ordinary parallel runner.
3. **Foreign-bundle relative links inside a moved Note are not re-resolved.** When a Note containing relative links authored by an external tool is moved, its *inbound* links are rewritten correctly, but relative links pointing *out* of it are left as written and may now resolve differently. No ticket's criteria required otherwise, and the behaviour is defensible under CAP-PORT-03's tolerance of externally-authored files — but it is a real gap, and the natural home for it is an expansion of CAP-PORT-03 rather than a bug fix.
4. **`find_notes_by_title`'s per-keystroke cost is unmeasured.** `schema.sql` declares no index on `notes(title)`, so the shipped query scans the active Workspace's rows. That is sound at current Note counts and was never on a keystroke path in this epic, since nothing calls it. Epic E's command-palette ticket puts it on one, and should measure it there before shipping — and add the index if the measurement calls for it.
5. **No Dart-side wrapper tests exist.** The guard against mis-wiring a regenerated binding lives entirely in the Rust wrapper-layer tests added by `WSPC-D008`'s fix round. A Dart-side test that a provider calls the FFI function it claims to would catch a class of error the Rust side structurally cannot see. Deferred to Epic E, which is where those wrappers first acquire a UI caller.

Three further items were carried into `tech-spec/changelog.md` v1.2.0 rather than here, because they are contract-shaped rather than scheduling-shaped: the `workspaces` cascade stranding `notes_fts` rows, `SpliceError` collapsing onto `AppError::ParseError`, and the absent batch/removal entry points on the incremental indexing path.
