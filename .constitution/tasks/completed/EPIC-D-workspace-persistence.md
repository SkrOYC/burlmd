...
2. **A known rare flake:** `git::operations::tests::path_in_head_distinguishes_a_new_note_from_one_already_in_history` fails roughly 1 run in 15. Pre-existing — it predates this epic and was not introduced by it — and not investigated here because no ticket scoped `git/operations.rs`'s test module. Whoever next touches that file should fix it rather than re-observe it.
