...
```text
Every declared candidate and gate has attributed evidence for syntax, positions, source fidelity, Links, rendered selections, structural edits, and Suggestions. The common functional matrix passes on all three managed roles. Performance and FFI projection evidence comes from `ubuntu-24.04` with 4 CPUs and 16 GB and Apple Silicon `macos-26` with 3 M1 CPUs and 7 GB. Apple Silicon `macos-15` contributes functional compatibility evidence only. Every role captures image, OS, architecture, CPU, memory, storage, viewport, build, corpus, run, and role identity in one complete role bundle. Candidate commands have no ambient GitHub, Git, package-registry, SSH-agent, or cloud credentials. Aggregation verifies the trust anchor, workflow signer, separate tested source, ticket base, AST allowlist, GitHub-hosted provenance, every bundled internal artifact, exact evidence classes, the prepared coordinator identity, and the credential-free nonnetwork sandbox before recommending one foundation or leaving the decision unresolved. The CLI stages verified role members under read-only fixed role directories and produces the authoritative schema-valid `results.json` in the sole writable output. The executor authors the human report from the accepted result and commits it with `results.json` and the accepted managed report.
```
...
```text
Generated and adversarial fixtures prove deterministic identity, collision freedom under all target equivalence rules, invertible ghost Links, safe case-only rename, and refusal of reserved, aliased, traversal, symlink, and submodule input. The common suite passes on all three managed roles. Authenticated GitHub-hosted Linux with ext4 and macOS 26 with APFS bundles bind to the same trust anchor, workflow signer, tested source, ticket base, build, corpus, and run identity. Aggregation rejects missing, incomplete, mismatched, stale, corrupt, self-hosted, unauthenticated, duplicate-operating-system, or filesystem-role evidence. Candidate commands have no ambient credential access. The accepted aggregate records unchanged trusted controls, PATH allowlist conformance, the prepared path-coordinator identity, and successful credential, descriptor, network, input-mount, and output-mount isolation. The CLI stages verified role members under the fixed role directories and produces the authoritative schema-valid `results.json`. The executor authors the human report from the accepted result and commits it with `results.json` and the accepted managed report.
```
