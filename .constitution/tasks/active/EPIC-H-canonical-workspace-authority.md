...
AST-H001 and PATH-H002 execute after both `CI-M003` pull requests merge. Their reports feed the constitution evidence-finalization pull request. Every dependent ticket must be reconciled against that accepted final TechSpec before implementation; the ticket stays in this epic and adapts rather than being discarded.

**Tranche integration:** Create `spike/epic-h-canonical-foundations` from merged `master` after both M bootstrap pull requests merge. Execute `AST-H001` and then `PATH-H002`, with one full milestone review after each ticket, and merge the H Spike pull request before constitution finalization. After the evidence-finalization pull request merges, create `feat/epic-h-authority-foundation` from merged `master` and execute `MODEL-H003`, `PATH-H005`, `ADAPT-H004`, `AUTH-H006`, and `PREFLIGHT-H007` in that order. Within either declared H tranche, each earlier milestone must be committed, validated, and independently reviewed before it satisfies the next ticket. Cross-tranche and cross-branch prerequisites must be merged into the base. Neither partial H tranche archives Epic H; `REPAIR-H008` through `RESCAN-H012` remain active.
...
- **Verification Command:** After the `CI-M003` evidence pull request merges and the tested source is pushed, run `TRUST_ANCHOR_WORKTREE/scripts/managed-evidence.sh run --ticket AST-H001 --trust-anchor-sha TRUST_ANCHOR_SHA --source-ref SOURCE_REF --tested-source-sha TESTED_SOURCE_SHA --base-sha BASE_SHA --output EVIDENCE_WORKTREE/.constitution/prototypes/ast/managed-evidence.json && git -C EVIDENCE_WORKTREE diff --check`. The clean detached trust-anchor client rejects any base-to-source change outside the AST write allowlist before dispatch and during collection. The trusted workflow runs `cargo test --locked --manifest-path .constitution/prototypes/ast/Cargo.toml --all-targets` on all three functional roles and the exact `SPK-AST-H001` Linux and macOS 26 probes in minimally privileged candidate jobs. Fresh dependent sealing jobs validate and authenticate the bundles. Before authentication, the client builds and identifies the locked AST coordinator. After verifying the two-job topology, unique candidate and sealed artifacts, and sealing provenance and tearing down credentials, it runs `SPK-AST-H001.coordinator_steps` with that prebuilt executable in the nonnetwork sandbox and validates `.constitution/prototypes/ast/results.json`.
...
- **STOP Conditions:**
  ...
  - STOP if a candidate command can observe a credential canary or OIDC or attestation capability; if the trusted upload wrapper lacks its job-scoped artifact runtime authority; if valid immutable untrusted candidate output doesn't cross into fresh seal; if that output is treated as trusted before seal; if candidate execution or processes gain seal OIDC or attestation authority or cross the fresh environment boundary; or if coordinator identity changes after preparation, isolation or network denial fails, an input is writable, or any path except the coordinator output is writable.
...
```text
Every declared candidate and gate has attributed evidence for syntax, positions, source fidelity, Links, rendered selections, structural edits, and Suggestions. All three roles provide `common-functional`. Linux and macOS 26 additionally provide `performance` and `ast-measurement`; macOS 15 provides no performance class. The AST profile doesn't claim platform-regression or authoritative visual evidence. Every candidate job captures image, OS, architecture, CPU, memory, storage, viewport, build, corpus, run, artifact nonce, and role identity in one complete untrusted bundle. Aggregation verifies that exact profile, the trust anchor, workflow signer, separate tested source, ticket base, AST allowlist, sealing provenance, every bundled internal artifact, the prepared coordinator identity, and the credential-free nonnetwork sandbox before recommending one foundation or leaving the decision unresolved. The CLI produces the authoritative schema-valid `results.json`; the executor commits its human interpretation with that result and the accepted managed report.
```
...
- **Scope (Out-of-Scope Files):**
  - Every repository path not listed above (don't touch production or active specifications)
- **Verification Command:** After the committed, validated, and independently reviewed `AST-H001` milestone and the PATH tested source are pushed, set `BASE_SHA` to the reviewed AST milestone. Run `TRUST_ANCHOR_WORKTREE/scripts/managed-evidence.sh run --ticket PATH-H002 --trust-anchor-sha TRUST_ANCHOR_SHA --source-ref SOURCE_REF --tested-source-sha TESTED_SOURCE_SHA --base-sha BASE_SHA --output EVIDENCE_WORKTREE/.constitution/prototypes/path/managed-evidence.json && git -C EVIDENCE_WORKTREE diff --check`. The clean detached trust-anchor client rejects any base-to-source change outside the PATH write allowlist before dispatch and during collection. The trusted workflow runs `cargo test --locked --manifest-path .constitution/prototypes/path/Cargo.toml --all-targets` on all three roles and the exact Linux ext4 and macOS 26 APFS probes in minimally privileged candidate jobs; fresh dependent sealing jobs validate and authenticate the bundles. After verifying the two-job topology, unique candidate and sealed artifacts, and sealing provenance and tearing down credentials, the client runs the prebuilt path coordinator in the nonnetwork sandbox and validates `.constitution/prototypes/path/results.json`.
...
```text
Generated and adversarial fixtures prove deterministic identity, collision freedom under all target equivalence rules, invertible ghost Links, safe case-only rename, and refusal of reserved, aliased, traversal, symlink, and submodule input. All three roles provide `common-functional`. Linux and macOS 26 additionally provide `filesystem-compatibility`; no role claims performance or visual evidence. Authenticated Linux ext4 and macOS 26 APFS bundles bind to the same trust anchor, workflow signer, tested source, ticket base, build, corpus, run identity, and artifact nonce. Aggregation rejects a missing assigned class, an unassigned extra class, incomplete or mismatched evidence, an invalid origin, or a filesystem-role error. The accepted aggregate records unchanged trusted controls, PATH allowlist conformance, candidate and sealing origins, the prepared path-coordinator identity, and successful coordinator isolation. The CLI produces the authoritative schema-valid `results.json`; the executor commits its human interpretation with that result and the accepted managed report.
```
