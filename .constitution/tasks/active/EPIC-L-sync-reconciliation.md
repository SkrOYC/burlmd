...
- **Verification Command:** After `CI-M003` merges and the source commit is pushed, run `./scripts/managed-evidence.sh run --ticket GIT-L001 --source-ref SOURCE_REF --source-head-sha SOURCE_HEAD_SHA --base-sha BASE_SHA --output .constitution/prototypes/git-analysis/managed-evidence.json && git diff --check`. The managed workflow runs the ordered `SPK-GIT-L001.verification_steps` on its declared Linux and macOS filesystem roles. Before authentication, the client builds and identifies the locked Git-analysis coordinator. After verified downloads and credential teardown, the CLI stages role contents, runs `SPK-GIT-L001.coordinator_steps` with that prebuilt executable in the nonnetwork sandbox, and validates `.constitution/prototypes/git-analysis/results.json` before accepting the evidence report.
