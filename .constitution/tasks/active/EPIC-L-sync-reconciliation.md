...
Deliver continuous Git synchronization and all three reconciliation forms: canonical-AST Suggestions, Lifecycle Decisions, and Asset Decisions. GIT-L001 runs after the `CI-M003` implementation and evidence pull requests merge so its cross-platform evidence uses the managed role workflows; every dependent ticket adapts to the measured Git protocol accepted by final TechSpec.
...
- **Verification Command:** After the `CI-M003` evidence pull request merges and the tested source is pushed, run `TRUST_ANCHOR_WORKTREE/scripts/managed-evidence.sh run --ticket GIT-L001 --trust-anchor-sha TRUST_ANCHOR_SHA --source-ref SOURCE_REF --tested-source-sha TESTED_SOURCE_SHA --base-sha BASE_SHA --output EVIDENCE_WORKTREE/.constitution/prototypes/git-analysis/managed-evidence.json && git -C EVIDENCE_WORKTREE diff --check`. The clean detached trust-anchor client rejects any base-to-source change outside the Git-analysis write allowlist before dispatch and during collection. The trusted workflow runs the ordered `SPK-GIT-L001.verification_steps` on its declared Linux and macOS filesystem roles in minimally privileged candidate jobs; fresh dependent sealing jobs validate and authenticate the bundles. Before authentication, the client builds and identifies the locked Git-analysis coordinator. After verifying the two-job topology, unique candidate and sealed artifacts, and sealing provenance and tearing down credentials, it runs the prebuilt coordinator in the nonnetwork sandbox and validates `.constitution/prototypes/git-analysis/results.json`.
...
```text
Every declared candidate and conflict/security gate has attributed structured evidence on both default Linux and macOS filesystems. All roles provide `common-functional`. Linux and macOS 26 additionally provide `filesystem-compatibility` and `git-protocol`; no role claims performance or visual evidence. The result tool captures host fingerprints, filesystem facts, and effective `core.ignoreCase`, `core.precomposeUnicode`, `core.protectHFS`, and filemode behavior from system and Git probes. Aggregation requires that exact profile and binds evidence to the trust anchor, workflow signer, tested source, ticket base, build, corpus, run, artifact nonce, sealing origin, and filesystem role. It rejects missing or unassigned classes, same-host, same-operating-system, role-mismatched, unauthenticated, or incomplete evidence. The accepted aggregate records unchanged trusted controls, Git-analysis allowlist conformance, candidate and sealing origins, the prepared coordinator identity, and successful coordinator isolation. The authoritative `results.json` contains exact object/tree identities, crash/CAS inputs, worktree nonmutation proof, and redistribution obligations. The executor commits its human interpretation with that result and the accepted managed report.
```
