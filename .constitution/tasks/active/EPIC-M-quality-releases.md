...
- **Verification Command:** Complete these steps in order:
  ...
  5. The trusted workflow runs `flutter test && dart analyze && flutter test integration_test -d linux -r github` on `ubuntu-24.04` inside the pipeline-owned headless environment. It runs `flutter test && dart analyze && flutter test integration_test -d macos -r github` on both Apple Silicon macOS roles.
...
The trusted `master` caller invokes `.github/workflows/ci-role-linux-x86-64.yml`, `.github/workflows/ci-role-macos-26-arm64.yml`, and `.github/workflows/ci-role-macos-15-arm64.yml` through static local paths. Each role workflow resolves from `WORKFLOW_SIGNER_SHA`; it checks out `TESTED_SOURCE_SHA` separately with persisted credentials disabled. The clean trust-anchor launcher creates expected identity and verifies that the signer retains every trusted control byte. Before dispatch and during collection, allowlist fixtures reject every extra, deleted, renamed, copied, mode-changed, symlink, or submodule path outside the ticket scope. Identity keeps the trust anchor, signer, tested source, base, and managed run nonce distinct. The local dispatch token has Actions write permission; the trusted workflows have only contents read, OIDC, and attestation permissions; collection is read-only. Candidate processes can't inherit those capabilities. Role fixtures reject cross-role evidence classes and unsafe internal paths. Bundle fixtures reject missing, extra, duplicate, escaped, size-mismatched, or digest-mismatched members. Aggregate fixtures prove `accepted` requires all roles, trust-anchor continuity, unchanged trusted controls, source-allowlist conformance, and successful checks with no reasons. Signed provenance must contain `runner_environment=github-hosted`; a self-hosted fixture with the expected labels is rejected. Typed fixtures include `trust-anchor-mismatch`, `candidate-control-surface-changed`, `source-allowlist-violation`, and every existing origin, role, artifact, coordinator, and aggregation rejection.
