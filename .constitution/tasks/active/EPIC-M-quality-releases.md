...
- **Verification Command:** Complete these steps in order:
  ...
  6. After the push run finishes, run `./scripts/managed-evidence.sh collect --source-ref SOURCE_REF --run-id RUN_ID --attempt RUN_ATTEMPT --source-head-sha SOURCE_HEAD_SHA --base-sha BASE_SHA --output .constitution/reports/ci-m003-managed-evidence.json && git diff --check`.
