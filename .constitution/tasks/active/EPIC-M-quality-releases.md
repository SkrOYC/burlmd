...
```text
Unique hashed and logged runs verify installed Git/runtime, secure storage, file selection, native loading, secret injection, update selection, host chrome, repeatability, and every named Linux candidate. The result tool obtains host fingerprints, operating systems, architectures, and versions from system APIs; command flags only assert expected values. The versioned build script runs `flutter clean`, enforces the committed dependency lock, and performs a release build for each construction. One macOS 26 host constructs the immutable artifact twice from identical inputs. Aggregation requires identical canonical path, mode, symlink-target, and file-byte manifests. The first construction is transferred with SHA-256 verification and probed on both the macOS 26 construction host and a distinct macOS 15 host. Aggregation requires both probes to name the same artifact SHA-256. Nix out-links are only build sources: the result tool copies their resolved bytes or closures into `artifacts/` inside the Spike allowlist before size and SHA-256 recording. The report selects or refuses a baseline.
```
