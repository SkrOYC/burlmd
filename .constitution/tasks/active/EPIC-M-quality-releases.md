...
- **Verification Command:** Complete these steps in order:
  ...
  5. In each role's minimally privileged `candidate` job, the trusted workflow runs `flutter test && dart analyze && flutter test integration_test -d linux -r github` on `ubuntu-24.04` inside the pipeline-owned headless environment. It runs `flutter test && dart analyze && flutter test integration_test -d macos -r github` on both Apple Silicon macOS roles. After candidate processes and their environment end, the dependent `seal` job validates and authenticates the bundle on a fresh environment with the same fixed role label.
...
- **Expected Success Output:** local validation exits 0; the reviewed implementation merge establishes `TRUST_ANCHOR_SHA`; all three roles finish every Flutter unit, analyzer, and desktop integration gate without credential access; trust-anchor, write-allowlist, descriptor-canary, namespace, mount, and representative-result fixtures pass; the dedicated evidence pull request contains only the accepted report and completion record and merges after independent review
- **STOP Conditions:**
...
- **Description:** Add one trusted `workflow_dispatch` caller and three distinct two-job reusable role workflows referenced through static local paths. Each role's `candidate` job has only contents read permission, checks out the separate tested SHA as data, runs candidate commands without artifact or signing credentials, closes candidate processes, and uploads one uniquely named untrusted bundle. Its dependent `seal` job starts on a fresh environment, validates that immutable bundle with trust-anchor code, and writes a trusted receipt for the verified job and candidate artifact facts. It alone receives OIDC and attestation authority to publish the uniquely named sealed bundle containing only that receipt and the unchanged candidate bundle. Implement `./scripts/managed-evidence.sh` with only the Stage 3 `collect` and `run` forms, the non-mutating generated-binding checker, and the managed-evidence isolation fixture. The launcher verifies its clean anchor checkout, trusted-control continuity, exact ticket source allowlist, exact job topology, and run-derived artifact names before dispatch and during collection. Add Linux-only Bubblewrap `0.11.2` through the locked Nix environment. For a managed Spike, the client prepares and identifies its locked coordinator before authentication, limits authenticated work to evidence verification, destroys authentication state, and then runs only the prebuilt coordinator in the nonnetwork sandbox. Reproduce the specified behavior without importing the Epic G branch. Pin `actions/checkout` to `fbc6f3992d24b796d5a048ff273f7fcc4a7b6c09`, `cachix/install-nix-action` to `13d8dd58da0234aa297dedd986986ccb8e7f3e24`, `actions/upload-artifact` to `043fb46d1a93c77aae656e7c1c64a875d1fc6a0a`, `actions/download-artifact` to `3e5f45b2cfb9172054b4087a40e8e0b5a5461e7c`, and `actions/attest` to `1e69f48acb82d1966a394da916b4c1698aa569d6`. Generate authoritative trust-anchor, workflow-signer, tested-source, base, release, build, corpus, run, artifact-nonce, required-role, and signer identity before fan-out. The coordinator verifies the distinct candidate and sealing jobs, unique candidate and sealed artifacts, GitHub-hosted sealing provenance, trusted receipt, run and attempt, upload and inner and outer bundle digests, and all bundled files before writing an accepted or rejected aggregate.
- **Acceptance:**
  - **Mode:** contract_test
  - **Evidence:**
