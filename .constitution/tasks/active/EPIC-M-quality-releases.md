...
**Bootstrap tranche:** Create `chore/epic-m-ci-foundation` from merged `master`. Execute `FLAKE-M002` and then `CI-M003`, with one full milestone review after each ticket, and merge the pull request before any managed-evidence tranche starts. This partial tranche doesn't archive Epic M. An unmerged branch or draft pull request never satisfies another ticket's dependency.
...
- **Verification Command:** After merged `CI-M003`, execute these exact commands in the managed role workflows, preserving order within each role. Use the authenticated two-phase artifact handoff instead of trusting run names, then run coordinator aggregation:
...
- **Verification Command:** In a disposable checkout, run the local contract gate: `cargo fmt --manifest-path rust/Cargo.toml -- --check && cargo clippy --workspace --all-targets --all-features --manifest-path rust/Cargo.toml -- -D warnings && cargo test --manifest-path rust/Cargo.toml && dart format --output=none --set-exit-if-changed lib test integration_test && ./scripts/check-generated-bindings.sh && flutter test && dart analyze && ! rg -n '\[DEBUG-' lib rust test scripts && actionlint && ./scripts/assert-ci-matrix.sh --workflow .github/workflows/ci.yml --require-runner ubuntu-24.04 --require-runner macos-26 --require-runner macos-15 --require-role-schema .constitution/tech-spec/contracts/ci-role-evidence.schema.json --require-aggregate-schema .constitution/tech-spec/contracts/ci-evidence.schema.json && git diff --check`. The candidate draft pull request must then complete all three role workflows and produce one schema-valid accepted aggregate.
- **Expected Success Output:** local validation exits 0; all three managed roles finish; the operator commits an aggregate that passes identity, origin, and integrity verification before milestone review
- **STOP Conditions:**
  - STOP if `FLAKE-M002` isn't committed and reviewed in `chore/epic-m-ci-foundation` or the branch wasn't created from merged `master`.
...
The generated-binding gate covers both generated sides and fails on drift. Because bootstrap uses the existing non-restoring checker, its disposable checkout absorbs mismatch changes. The candidate workflow never pushes repository content, publishes a release, or receives live product credentials. An operator commits the accepted aggregate; only that commit can start the full `CI-M003` milestone review. Final Stage 3 must revalidate the generator triple and generated output before downstream production tickets execute.
```
