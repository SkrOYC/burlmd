...
Linux static and integration fixtures prove `env -i` Bubblewrap `0.11.2` PID containment by rejecting a surviving double-fork probe before upload. Hosted-macOS fixtures distinguish candidate-command no-privilege execution from the trusted wrapper's later artifact runtime authority. They prove fresh-job separation, seal-only OIDC and attestation authority, safe validation, and no candidate-byte execution in seal. A surviving candidate process can corrupt untrusted output or cause a fail-closed upload denial. It can't gain seal authority or enter the fresh seal environment. Fixtures reject any universal macOS containment or zero-survivor claim. Accepted evidence authenticates reviewed workflow execution and sealed provenance. It does not prove that arbitrary malicious macOS candidate code is contained or that its untrusted output is truthful. Reviewed source and test contracts remain required.
