...
- **Verification Command:** After `CI-M003` merges and the source commit is pushed, run `./scripts/managed-evidence.sh run --ticket PKG-M001 --source-ref SOURCE_REF --source-head-sha SOURCE_HEAD_SHA --base-sha BASE_SHA --output .constitution/prototypes/packaging/managed-evidence.json && git diff --check`. The managed workflows run these exact commands, preserving order within each role:
