...
- **Verification Command:** After the `CI-M003` evidence pull request merges and the tested source is pushed, run `TRUST_ANCHOR_WORKTREE/scripts/managed-evidence.sh run --ticket ASSET-I001 --trust-anchor-sha TRUST_ANCHOR_SHA --source-ref SOURCE_REF --tested-source-sha TESTED_SOURCE_SHA --base-sha BASE_SHA --output EVIDENCE_WORKTREE/.constitution/prototypes/assets/managed-evidence.json && git -C EVIDENCE_WORKTREE diff --check`. The clean detached trust-anchor client rejects any base-to-source change outside the Asset write allowlist before dispatch and during collection. The trusted workflow runs `cargo test --locked --manifest-path .constitution/prototypes/assets/Cargo.toml --all-targets` on all three functional roles and the exact Linux x86-64 and macOS 26 performance probes in minimally privileged candidate jobs; fresh dependent sealing jobs validate and authenticate the bundles. Before authentication, the client builds and identifies the locked Asset coordinator. After verifying the two-job topology, unique candidate and sealed artifacts, and sealing provenance and tearing down credentials, it runs the prebuilt coordinator in the nonnetwork sandbox and validates `.constitution/prototypes/assets/results.json`.
- **Expected Success Output:** exit 0 with `.constitution/prototypes/assets/results.json`, accepted `.constitution/prototypes/assets/managed-evidence.json`, executor-authored `.constitution/spikes/SPK-ASSET-I001.md`, and explicit OD-06/OD-07 disposition
- **STOP Conditions:**
  - STOP if credentials would enter committed files or evidence.
  - STOP if a result weakens Protected State or deletion safety.
  - STOP if the `CI-M003` evidence pull request isn't merged or if evidence is missing, stale, mismatched, corrupt, unauthenticated, or assigned to the wrong role.
  ...
  - STOP if the launcher isn't the clean trust-anchor copy, a trusted control file differs at the signer SHA, or the base-to-source diff leaves the Asset write allowlist.
...
```text
Candidate-attributed results cover all three common functional roles, both performance roles, every declared gate, exact tool and service configurations without secrets, UI Responsiveness and Idle Memory disposition, and a recommendation or explicit unresolved outcome. Linux x86-64 `ubuntu-24.04` supplies performance evidence with 4 CPUs and 16 GB; Apple Silicon `macos-26` supplies performance evidence with 3 M1 CPUs and 7 GB; `macos-15` is functional compatibility-only. Each minimally privileged candidate job captures the verified logical viewport and complete image, environment, build, corpus, run, nonce, and role identity in one untrusted bundle. A fresh sealing job validates and authenticates the unique sealed artifact. Aggregation rejects a self-hosted origin or a missing, incomplete, stale, mismatched, corrupt, unauthenticated, duplicate, invalidly named, failed-candidate, missing-seal, or role-invalid result. The accepted aggregate records the trust anchor, workflow signer, separate tested source, Asset allowlist conformance, distinct candidate and sealing origins, prepared coordinator identity, and successful credential, descriptor, network, input-mount, and output-mount isolation. The CLI stages verified sealed role members under the fixed role directories and produces the authoritative schema-valid `results.json`. The executor authors the human report from the accepted result and commits it with `results.json` and the accepted managed report.
```
