...
Evolution pass for PRD v1.3.7, Architecture v1.4.15, and TechSpec v1.8.0-provisional. The backlog remains 80 tickets and 564 active story points. The dependency graph now has 225 edges, and the recomputed critical path is 180 points.
...
- Replaced physical workstation profiles with managed `ubuntu-24.04`, Apple Silicon `macos-26`, and Apple Silicon `macos-15` roles. Linux and macOS 26 own performance evidence, Linux uses private headless exact visual capture, macOS 26 owns the macOS baseline, and macOS 15 remains functional compatibility-only.
- Updated CI, AST, path, Asset, observer, packaging, benchmark, shell, and installed-candidate evidence to use authoritative expected identity, immutable pre-upload role manifests, authenticated artifact handoff, and accepted or rejected aggregation.
