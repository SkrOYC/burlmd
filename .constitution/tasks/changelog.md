...
- Split each managed role into a minimally privileged candidate job and a fresh dependent sealing job. Candidate output is one complete untrusted bundle containing its manifest and every named result, log, raw measurement, and handoff. Only seal receives OIDC and attestation authority. Acceptance requires signed `runner_environment=github-hosted` provenance, run-derived artifact-safe nonces, and unique expected, candidate, and sealed names; it rejects self-hosted evidence, collisions, and substitutions.
