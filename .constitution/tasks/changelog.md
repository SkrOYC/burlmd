...
- Added the user-authorized isolated tranche sequence: M bootstrap, H Spikes, constitution evidence finalization, H authority foundation, and surgical Epic G replay. Every tranche starts from merged `master`, uses its own worktree, branch, and pull request, and runs one full review per ticket milestone.
- Added exact ownership for `chore/epic-m-ci-foundation`, `spike/epic-h-canonical-foundations`, and `feat/epic-h-authority-foundation`. Partial H and M tranches don't archive their epics. Cross-tranche and cross-branch prerequisites must be merged into the base; within a declared tranche, an earlier committed, validated, and independently reviewed milestone satisfies the next ticket.

### Changed

- Replaced the global all-five-Spike production stop with contract-scoped authorization. The six Epic G M0 tickets remain executable; `FLAKE-M002` and `CI-M003` are executable bootstrap work. Every other production ticket waits for its own evidence, required upstream finalization, and Stage 4 adaptation.
- Made `CI-M003` the merged prerequisite for managed-evidence Spikes. `AST-H001` and `PATH-H002` run sequentially in the H Spike tranche; the H foundation then runs `MODEL-H003`, `PATH-H005`, `ADAPT-H004`, `AUTH-H006`, and `PREFLIGHT-H007` in order.
- Replaced physical workstation profiles with managed `ubuntu-24.04`, Apple Silicon `macos-26`, and Apple Silicon `macos-15` roles. Linux and macOS 26 own performance evidence, Linux uses private headless exact platform-regression capture, macOS 26 owns the sole authoritative product visual baseline, and macOS 15 remains functional compatibility-only.
- Updated CI, AST, path, Asset, observer, packaging, benchmark, shell, and installed-candidate evidence to use authoritative expected identity, immutable pre-upload role bundles, authenticated artifact handoff, and accepted or rejected aggregation.
- Authorized the provisional Flutter Rust Bridge `2.12.0` generator and runtime triple only for CI bootstrap in a disposable checkout. Downstream production remains blocked until final Stage 3 revalidates the triple and generated bytes.
...
- Split each managed role into a minimally privileged candidate job and a fresh dependent sealing job. The separate sealing receipt contains immutable in-job facts and `job.check_run_id`, but no final state. After the attempt completes, aggregation resolves that locator and requires one successful API job. Artifact checks normalize the pinned action's bare 64-hex digest to the REST and aggregate `sha256:<hex>` form and reject mismatches.
- Assigned exact evidence classes per ticket and role. CI bootstrap proves functional, generated-binding, analysis, integration, protocol, security, and isolation outcomes without claiming performance or visual evidence. The five Spikes require only their measured AST, filesystem, Git, Asset, or packaging classes. Later benchmark, shell, and release tickets retain their own performance and visual gates.
