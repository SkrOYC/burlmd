...
One note on scope against the ticket's own acceptance language: the Gherkin criterion asks for rendered screenshot comparisons, while this report delivers measured rendered geometry instead — `RenderBox.size` and `getBoxesForSelection` line counts and offsets rather than pixel images. That substitution satisfies the intent, because the STOP condition demands inspected rendered output over widget-property assertions, and numeric measurements taken from the live render tree are stronger evidence than eyeballed pixels — they caught in this very report (the implicit-`height` jump) what a screenshot comparison might plausibly have waved through as "close enough". `EDIT-F002`'s mandatory smoke-shot screenshot supplies the production-font visual confirmation on top.
