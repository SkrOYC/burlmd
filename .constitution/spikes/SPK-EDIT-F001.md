...
The selection result is also regression-tested through a real mouse drag: a pointer sequence that starts in a focused `EditableText` cannot issue a Core range operation even if it blurs while travelling outward. A fresh pointer-down and rendered drag after `commit_block` is required before Core copy can receive a `BlockRange`. This makes the focused interior case impossible by contract as well as by Flutter's current selection behavior.
