...
The final release contract must identify version, source revision, platform, architecture, artifact kind, checksum, minimum runtime, signature state, feature-matrix result, and GitHub Release URL. Managed validation uses authoritative release, build, corpus, run, required-role, and role-signer identity. The pre-upload `ci-role-evidence.schema.json` contract records one role's hosted image, system facts, gate results, and internal artifact hashes without service-assigned upload fields. The final `ci-evidence.schema.json` contract adds authenticated workflow origin, artifact API identity and digest, and a discriminated accepted or rejected aggregate. SPK-PKG-M001 settles the Linux runtime values and construction inputs. Signing remains explicitly absent during `0.x`.
