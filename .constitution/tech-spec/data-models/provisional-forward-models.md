...
The final release contract must identify version, source revision, platform, architecture, artifact kind, checksum, minimum runtime, signature state, feature-matrix result, and GitHub Release URL. The authoritative managed-validation identity, origin, coordinator-execution identity, and isolation-result shapes are defined only by `contracts/ci-role-evidence.schema.json` and `contracts/ci-evidence.schema.json`; this summary doesn't duplicate their fields. Those schemas keep the immutable trust anchor, trusted workflow signer, separate tested source, and ticket base distinct. The raw Spike contract defines the source write allowlist and trusted validation controls. For each managed Spike, `contracts/provisional-spikes.toml` declares the accepted evidence report, authoritative machine result, human report, fixed coordinator root, and ordered coordinator steps. Verified role-bundle members are staged under role-specific directories before credential-free aggregation. The CLI produces and validates the machine result, while the executor authors its human interpretation. SPK-PKG-M001 settles the Linux runtime values and construction inputs. Signing remains explicitly absent during `0.x`.
