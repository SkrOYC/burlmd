...
A file that is missing frontmatter, or whose frontmatter does not parse, is **not** rejected — it is indexed with a derived title and reported as non-conformant, so that CAP-PORT-03 (tolerating files written by external tools) and CAP-WS-05 (opening a foreign Workspace) both hold.
