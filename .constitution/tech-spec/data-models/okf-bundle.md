...
A file that is missing frontmatter, whose frontmatter does not parse, or whose frontmatter parses without a non-empty `type`, is **not** rejected — it is indexed with a derived title and reported as non-conformant. All three cases, because OKF §11 states exactly two conformance conditions and the second is `type`: a block containing only `title:` parses perfectly and is still non-conformant, so that CAP-PORT-03 (tolerating files written by external tools) and CAP-WS-05 (opening a foreign Workspace) both hold.
