...
-- file exists on any machine, so the v1.1.0 corrections below are edits to the
...
CREATE TABLE IF NOT EXISTS notes (
    ...
    -- (OKF v0.2 SPEC.md section 2). Workspace-relative, '/'-separated, no
    -- leading slash. NOT a UUID -- the previous claim that this column held
    -- "a stable UUID persisted in Markdown YAML frontmatter" was never
    -- implemented and contradicted both the specification and the code; see
    -- ADR-004. Because identity is positional, a rename or move rewrites this
    -- value and every `links.target_id` pointing at it (CAP-LIFE-02/03).
    id TEXT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS links (
    ...
    -- whose target_id matches no `notes.id` -- which OKF section 6.1 requires
    -- consumers to tolerate, and which CAP-GRAPH-04 makes a feature.
    ...
    -- Deliberately no foreign key on target_id, to permit ghost links.
);

-- Backlink lookups (CAP-GRAPH-05) query by target. Without this index that is
-- a full scan of every edge in the Workspace on every Note open.
...
-- `fts_mapping` alongside it.
CREATE VIRTUAL TABLE IF NOT EXISTS notes_fts USING fts5(
    title,
    content
);

-- Maps a Note's concept id to its FTS5 internal rowid, for O(1) deletion.
...
-- transient: it is cleared once tier 2 writes the Note to disk.
CREATE TABLE IF NOT EXISTS drafts (
    note_id TEXT PRIMARY KEY,
    workspace_id TEXT NOT NULL,
    raw_markdown TEXT NOT NULL,    -- Full current source text of the Note
    updated_at INTEGER NOT NULL
    -- No foreign key to notes(id): a new Note exists as a draft before its
    -- file, and therefore its `notes` row, exists.
);
...
CREATE TABLE IF NOT EXISTS directories (
    id TEXT PRIMARY KEY,           -- Workspace-relative path, matching `path`
    workspace_id TEXT NOT NULL,
