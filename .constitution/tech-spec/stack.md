...
- **Android SDK:** Pinned via `devenv`'s `android` module (platforms 34/35, build-tools 35.0.0, NDK 28.2) even though mobile targets are deferred. This is a reproducibility measure, not a scope change: absent an explicit `ANDROID_HOME`, Flutter discovers an SDK by scanning the contributor's home directory, which would leave the mobile toolchain unpinned and unversioned the moment mobile is unshelved. `flutter doctor` reports `Android license status unknown` regardless, because `sdkmanager` cannot persist a license acceptance into the read-only Nix store; this does not affect builds.
...
- **FRB Version Triple:** `flutter_rust_bridge_codegen` (pinned by `nixpkgs`, currently 2.12.0), the `flutter_rust_bridge` Rust crate, and the `flutter_rust_bridge` Dart package must all agree on version. The codegen is pinned by Nix while the other two resolve from crates.io and pub.dev, so a `devenv update` that moves the codegen requires bumping the other two in lockstep.
