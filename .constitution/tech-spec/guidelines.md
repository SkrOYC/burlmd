...
Before dispatch, the launcher creates canonical `expected-identity.json` bytes. The identity separates `trustAnchorSha`, `workflowSignerSha`, `testedSourceSha`, `baseSha`, `runIdentity`, and its bijectively derived `artifactNonce`. It also binds the exact source write allowlist. The launcher passes the base64 bytes and their SHA-256 digest as dispatch inputs and retains its local bytes. The trusted caller decodes those exact bytes for role fan-out and its expected-identity artifact. The workflow doesn't reconstruct candidate-supplied identity.

The trusted caller and its three static local reusable workflows run from `WORKFLOW_SIGNER_SHA` on `master`. Each role checks out `TESTED_SOURCE_SHA` into a separate data directory with `persist-credentials: false`. It removes checkout authentication before candidate execution. It never loads a workflow, action, launcher, or configuration from the tested checkout. Artifact names use only fixed lowercase prefixes, the fixed role, and `ARTIFACT_NONCE`: `managed-evidence-expected-{artifactNonce}`, `managed-evidence-candidate-{role}-{artifactNonce}`, and `managed-evidence-sealed-{role}-{artifactNonce}`. This alphabet excludes the pinned upload action's forbidden double quote, colon, angle brackets, vertical bar, asterisk, question mark, carriage return, line feed, backslash, and forward slash. Collection rejects a run/nonce mismatch, a forbidden character, or any missing, duplicate, extra, or substituted reserved name.
...
Each role workflow has two static jobs on separate fresh hosted environments. The `candidate` job receives only `contents: read`; all unspecified permissions, including `id-token`, `attestations`, and `actions`, are `none`. A trusted launcher runs every tested-source build, test, probe, and packaging command with owned empty user and configuration directories, closed inherited descriptors, and an explicit environment that omits GitHub tokens, Actions artifact runtime tokens and URLs, OIDC request variables, SSH-agent and askpass state, package-registry tokens, cloud credentials, and Git credential configuration. Canary fixtures prove a representative candidate command can't observe those capabilities and still succeeds.

After candidate commands end, the candidate job terminates and verifies their process tree. It creates `ci-role-evidence.json` and the complete deterministic `ci-role-evidence.tar.zst` as untrusted output. The trusted workflow wrapper invokes the pinned upload action with `overwrite: false` and `if-no-files-found: error` for the exact unique candidate artifact name. It exports the action's artifact ID and digest as trusted job outputs. Any failed command, cleanup, bundle creation, or upload fails the candidate job.

The `seal` job has `needs: candidate` without an `always()` override and starts on a fresh hosted environment with no shared process namespace, workspace, or writable filesystem. Detached candidate processes and candidate filesystem mutations end with the candidate environment. Only the immutable candidate artifact crosses the boundary. The sealing job downloads the exact unique name and uses only immutable trust-anchor validators to check expected identity, manifest shape, every internal hash, the artifact name, the trusted upload outputs, and the exact run's candidate and sealing job records. It rejects an unexpected member, changed bundle, duplicate reserved name, pre-upload collision, failed candidate, missing seal, equal job IDs, wrong label, or self-hosted label. It never executes candidate bytes.

Only `seal` receives `actions: read`, `contents: read`, `id-token: write`, and `attestations: write`. After validation, it writes trusted `ci-seal-receipt.json` from the verified candidate and sealing job records and candidate artifact ID, name, and digest. It creates deterministic `ci-sealed-role-evidence.tar.zst` containing only that receipt and the unchanged candidate `ci-role-evidence.tar.zst`. It attests the sealed bundle and uploads it exactly once under the unique sealed artifact name with `overwrite: false` and `if-no-files-found: error`. The receipt contains no sealed artifact or attestation field, so it doesn't create a digest cycle. A candidate attempt to reserve that name makes the immutable upload fail, but candidate processes lack the runtime artifact credentials needed to upload it. This fresh-environment boundary, rather than a platform sandbox, keeps candidate processes and filesystem state away from OIDC and attestation steps. Bubblewrap remains the separate post-download coordinator boundary.

Release identity is `candidate:` followed by the 40-hex tested source SHA. Build identity is the SHA-256 of an exact input manifest containing the trust anchor, workflow signer, tested source, source diff, base SHA, repository tree, lockfiles, toolchain pins, action pins, and build configuration. Corpus identity is the SHA-256 of the generated corpus manifest. Run identity is `managed:` plus the launcher's lowercase 32-hex nonce. Artifact nonce is exactly the same 32-hex suffix; the service run ID and attempt remain separate origin fields. Required roles are exactly `linux-x86_64`, `macos-26-arm64`, and `macos-15-arm64`. The expected identity also assigns one signer to each role:
...
The authenticated client enumerates the exact run's artifacts and job records. For each role it requires the static two-job topology, distinct numeric job IDs, successful conclusions, fixed hosted labels without `self-hosted`, one exact candidate artifact, and one exact sealed artifact. The candidate artifact's service ID and digest must equal the trusted upload outputs. The sealed artifact must authenticate the `seal` workflow and its signed `runner_environment=github-hosted` origin. The client downloads only the sealed object, verifies its service digest and attested subject, and safely unpacks exactly `ci-role-evidence.tar.zst` and `ci-seal-receipt.json`. It compares every receipt field with the run APIs, expected identity, and inner bundle digest. It then safely unpacks the inner bundle, validates the manifest, and verifies every `internalArtifacts` entry before using a result. A missing, extra, duplicate, escaped, size-mismatched, or digest-mismatched outer or inner member rejects the role bundle.
...
The signed provenance must bind the sealing job's exact static reusable-workflow path, `workflowSignerSha`, `refs/heads/master`, invocation run and attempt, and `runner_environment` claim. That claim must equal `github-hosted`; a self-hosted sealing job is rejected even if it supplies the expected custom labels. The aggregate retains separate candidate job ID, labels, environment, and artifact facts and sealing job ID, labels, environment, signer, attestation, and artifact facts. The versioned job API corroborates the exact static two-job structure in the same run. The unique run-derived artifact names, trusted candidate upload outputs, `needs` edge, distinct fresh job IDs, and signed sealing provenance form the correlation; the client never accepts a manifest-supplied job identity.
