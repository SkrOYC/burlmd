...
TechSpec v2.1.23 (translated from reviewed v1.8.7-provisional) permits research code only under `.constitution/prototypes/`. The Epic G M0 production exceptions remain unchanged. `BURL-M015` and `BURL-M003` may write production code only for their reproducibility and validation bootstrap. Stage 4 must adapt `BURL-M003` to raw contract version `38`. It must maintain the `BURL-O001` Stage 3 stop without authorizing implementation. Every other production ticket remains blocked by its own decision evidence and matching Stage 3 and Stage 4 adaptation. Except for these contract-scoped exceptions, production directories (`lib/`, `rust/`, `linux/`, and `macos/`) are read-only inputs to this research wave.
...
The `scripts/supervise-linux-session.sh` file defines its constants, helpers, and one complete `main` function before the final top-level `main "$@"` call. At the start of `main`, after argument parsing but before the first descriptor inventory, run the Bash builtin `exec 255<&-`. Require descriptor 255 to be absent. Every base path ends with the exact candidate `exec`; integration and error paths exit from `main`. Don't return to top-level script reading or rely on `CLOEXEC`. A pinned Bash 5.3p9 fixture must exercise both exit and final-exec paths, preserve meaningful standard output and standard error, and prove unchanged standard-stream identities.

Before pipe creation, the trusted launcher establishes the candidate's final standard streams as descriptors 0, 1, and 2. The Bubblewrap branch closes the parent-retained teardown-lock descriptor. It receives only descriptor 3, the preflight-record write end, and descriptor 4, the acknowledgement read end. The parent closes those child ends, and the launch branch closes the opposite parent ends before Bubblewrap starts. After `main` explicitly closes the script descriptor, the preflight enumerates exactly descriptors 0 through 4 with those roles. It writes `preflight-bytes=<canonical-decimal>`, one line feed, and the declared body to descriptor 3, then closes descriptor 3.
...
This closure view authorizes only `BURL-M003`. Stage 4 must add the retained-descriptor teardown verification and the post-reap cleanup-descriptor transition within that ticket's existing launcher, helper, and test paths. For each candidate namespace and session payload, the parent launches only Bubblewrap. The existing trusted preparation, identity checks, and pinned post-session `flock` verifier remain required and don't form a general process allowlist. The adaptation record lists `BURL-M003` under `affected_tickets` and `BURL-O001` under `maintained_stop_tickets`. The latter preserves the Stage 3 stop without authorizing `BURL-O001` implementation.
