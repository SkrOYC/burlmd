...
verification_commands:
  ...
  - name: test
    ...
    exists: true
  - name: format
    label: transition command quoted by BURL-M003 in Tasks v3.2.12
    command: "cargo fmt --manifest-path rust/Cargo.toml -- --check && cargo clippy --workspace --all-targets --all-features --manifest-path rust/Cargo.toml -- -D warnings && cargo test --manifest-path rust/Cargo.toml && dart format --output=none --set-exit-if-changed lib test test_driver integration_test && ./scripts/check-generated-bindings.sh && flutter test && dart analyze && ! rg -n '\\[DEBUG-' lib rust test scripts && actionlint && ./scripts/assert-ci-matrix.sh --workflow .github/workflows/ci.yml --require-runner ubuntu-24.04 --require-runner macos-26 --require-runner macos-15 --require-role-schema .constitution/tech-spec/contracts/ci-role-evidence.schema.json --require-aggregate-schema .constitution/tech-spec/contracts/ci-evidence.schema.json && ./scripts/assert-managed-evidence-isolation.sh --contract .constitution/tech-spec/contracts/provisional-spikes.toml --sandbox bubblewrap --expected-version 0.11.2 && ./scripts/test-managed-role-production-contract.sh && ./scripts/test-managed-evidence-reconciliation.sh && ./scripts/test-seal-validators.sh && git diff --check"
    exists: false
    owner: BURL-M003
  - name: format
    label: canonical raw-39 verification for BURL-M003 after Tasks adaptation
...
TechSpec v2.1.28 (translated from reviewed v1.8.7-provisional) permits research code only under `.constitution/prototypes/`. The Epic G M0 production exceptions remain unchanged. `BURL-M015` and `BURL-M003` may write production code only for their reproducibility and validation bootstrap. Raw contract version `39` requires a Stage 4 adaptation of `BURL-M003` and the downstream Tasks that restate the managed Linux label or profile. `BURL-O001` remains a maintained stop and isn't authorized. In the same documentation pull request, a separately validated and independently reviewed Stage 4 commit must apply the actual Tasks changes. A later reviewed Stage 3 acknowledgement commit may mark `ci_bootstrap.stage4_tasks_adaptation.status` as `satisfied` only by citing that exact Tasks commit and version. The recovery implementation remains blocked until the entire reviewed documentation pull request merges. Every other production ticket remains blocked by its own decision evidence and matching Stage 3 and Stage 4 adaptation. Except for these contract-scoped exceptions, production directories (`lib/`, `rust/`, `linux/`, and `macos/`) are read-only inputs to this research wave.
...
Merge the complete reviewed documentation pull request before implementation. Create one implementation commit on `fix/epic-m-hosted-runner-recovery` from that merged documentation tip. The commit receives local validation, independent milestone review, and pull-request review. Rebase-merging the pull request makes the resulting `master` tip the replacement `TRUST_ANCHOR_SHA`. Set `BASE_SHA` to its first parent, which is the merged recovery-documentation tip. If that reviewed recovery later fails under the same raw-version-39 contract, apply each correction as one separately reviewed commit from the immediately preceding reviewed `master` tip. That preceding tip becomes `BASE_SHA`, and the correction's merged tip becomes the next replacement anchor. Every base must retain the merged approved documentation and the original PR #15 and BURL-M015 ancestry. Never use an arbitrary ancestor or combine several correction commits in one source range. The exact `BASE_SHA..TRUST_ANCHOR_SHA` range contains one reviewed implementation commit and must satisfy the unchanged `bootstrap_write_allowlist`. Every anchor requires fresh self-validation and the dedicated reviewed evidence pull request.
...
For BURL-M003, preserve the original nonzero status and run the existing bounded cleanup after a macOS candidate gate or integration invocation fails. The trusted wrapper can emit one failure-only diagnostic block to the GitHub Actions step log. Select the source log only from a fixed trusted gate-to-path mapping under the canonical role-output root. Never use a candidate-provided path.

Open the fixed log read-only with no-follow and nonblocking flags. Require `fstat` to identify a regular file before reading. Reject a symbolic link, FIFO, socket, device, directory, or another special file without reading it. Record the fixed gate ID, original exit status, observed source-log size from `fstat`, retained-tail byte count, and SHA-256 of only the retained tail. From the observed size, copy at most the final 65,536 bytes into a fixed-size wrapper-owned regular-file snapshot. Hash and encode only that snapshot. Never scan or hash the complete source log.

Encode every retained byte as base64. Prefix every emitted line with a fixed trusted literal. Never emit candidate bytes directly or treat them as shell, workflow-command, `GITHUB_OUTPUT`, `GITHUB_ENV`, or job-summary input. The diagnostic is untrusted, makes no complete-log integrity claim, and cannot affect acceptance. Don't upload it or add it to an accepted artifact inventory. If the source is unavailable, truncated, replaced, or changing, the wrapper can suppress the diagnostic. Diagnostic failure must not replace the original candidate status or cleanup result.

The implementation must keep fail-closed `errexit` behavior. Don't place `candidate_phase` or another multi-check function in an `if`, `!`, `&&`, or `||` context that disables `errexit` throughout the function. A pinned Bash fixture must reproduce the actual nested path: `candidate_exec` enables `errexit` before returning a nonzero child status, `run_ci_gate` retains that status and reaches diagnostic recording, and the top-level `candidate_phase` call exits before a later gate runs. The fixture must cover oversized, sparse, and changing regular files plus a symbolic link, FIFO, directory, and another special file when available. It must prove the bounded snapshot, safe encoding of workflow-command payloads, no blocking or unsafe read, cleanup, and original-status preservation.
