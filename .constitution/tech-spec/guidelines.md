...
TechSpec v2.1.11 (translated from reviewed v1.8.7-provisional) permits research code only under `.constitution/prototypes/`. The existing Epic G M0 production exceptions remain unchanged. `BURL-M015` and `BURL-M003` may write production code only for their reproducibility and validation bootstrap. Tasks v3.2.10 adopted the v2.1.9 receipt-integrity delta. Stage 4 must adapt `BURL-M003` and `BURL-O001` directly to raw contract version `37` before implementation continues. The patch must include the protected capacity transcript, exact schema and phase ordering, finalization coverage, corrected thresholds, host-to-private database transfer, prefetched Flake requisites, and successful-only failure behavior. Every other production ticket remains blocked by its own decision evidence and a matching Stage 3 and Stage 4 adaptation. Except for these contract-scoped exceptions, production directories (`lib/`, `rust/`, `linux/`, and `macos/`) are read-only inputs to this research wave.
...
The trusted control inventory also includes `contracts/linux-capacity-record.schema.json`. The build-input manifest hashes its exact trust-anchor bytes and version. The Linux wrapper, fresh seal, and final aggregator recompute that hash from their clean trusted checkout. They don't accept a schema or digest from candidate input, the tested-source checkout, or an unsealed artifact.
...
Candidate output is untrusted. On Linux, the trusted launcher starts candidate commands with `env -i` in a Bubblewrap `0.11.2` private PID namespace. It must prove that a double-fork probe does not survive the teardown lock before finalization or upload. Any failed command, required Linux teardown, capacity check, bundle creation, or upload fails the candidate job. On hosted macOS, the launcher terminates and reaps known test processes and records cleanup failure. That bounded cleanup does not claim zero survivors or lifecycle containment for arbitrary candidate code. The candidate job then creates `ci-role-evidence.json` and the complete deterministic `ci-role-evidence.tar.zst` as untrusted output. After candidate commands finish, the trusted workflow wrapper invokes the pinned upload action with `overwrite: false` and `if-no-files-found: error` for the exact unique candidate artifact name. The wrapper necessarily receives job-scoped artifact runtime authority for that upload. A surviving hosted-macOS process can corrupt this untrusted output or cause an upload denial. It cannot gain seal-only OIDC or attestation authority, and it cannot enter the fresh sealing environment. The wrapper exports the action's artifact ID and bare lowercase 64-hex `artifact-digest` as trusted job outputs.

ADR-020 defines the assumed Linux closure-view decision. The trusted launcher derives a sorted manifest with `nix-store -qR` from the ticket's declared tool roots. It verifies that every direct Nix reference remains in that manifest. Each manifest member must be an existing, canonical, nonsymlink `/nix/store` entry. Internal links remain intact inside the mounted entry.

Bubblewrap starts with an empty root. For profiles without Nix command authority, the launcher creates empty mount points and binds that owned store root read-only at `/nix/store`. It then adds one `--ro-bind HOST_PATH HOST_PATH` for each manifest member. It never mounts the host store root, host Nix state, or daemon socket. Before the first candidate command, the visible store entries must equal the manifest. An unlisted host-store canary must be absent. Both writes to mounted members and creation of another store entry must fail. Executable fixtures must run the pinned script interpreters, ELF loaders, runtime search path libraries, compiler tools, and desktop helpers from this view.

The launcher mounts the manifest read-only at `/contract/locked-nix-closure.manifest`. Candidate assertions consume that file instead of a colon-delimited environment value. Each Linux namespace adds `--disable-userns` and `--assert-userns-disabled`; assertions don't create nested Bubblewrap sessions. Descriptor closure, network unsharing, private loopback, the PID namespace, and the teardown lock remain unchanged.

Before `exec`, the launcher counts the complete NUL-inclusive environment and argument bytes. This count includes every closure bind and the candidate command. The total must remain below half of `getconf ARG_MAX`. The same limit applies to the `nix-store --dump-db` argument vector. The pinned M003 prototype uses 75,208 bind-argument bytes for 556 paths against an `ARG_MAX` of 2,097,152 bytes.

Linux profiles without Nix command authority receive no Nix database. The closure manifest and executable checks are sufficient because these profiles run realized paths only. `BURL-O001` needs a writable local store for offline build outputs. It requires Nix 2.35.2 from the pinned install action.

Before network removal, the host wrapper archives every locked Flake input, verifies that `flake.lock` remains unchanged, and realizes every input derivation for the selected package. It runs `nix-store -qR --include-outputs` only on those input derivations. The final manifest contains their complete requisite set, the selected target derivation, and every launcher root. A selected target output in the manifest rejects the run, so the private build can't become a host-cache no-op. Missing, invalid, or out-of-manifest requisites also reject the run.

Before namespace entry, the host wrapper exports exactly the final manifest paths with `nix-store --dump-db PATHS`. It writes the dump under the protected contract root. The namespace mounts that dump read-only. Inside the namespace, the root process loads it with `nix-store --load-db` into the fresh private database mounted at `/nix/var/nix`. The private output store is mounted at `/nix/store`, and each host manifest member overlays its canonical path read-only. `NIX_REMOTE=local`, `sandbox = false`, an empty `build-users-group =`, empty substituters, an empty registry, no network, and no daemon socket are mandatory. The candidate can register private derivations and outputs, but it can't write host closure bytes or query ambient host-store metadata.

Capacity checks use allocated filesystem bytes rather than Nix Archive (NAR) size. Before any measured output, the launcher inventories canonical, nonoverlapping roots on one filesystem. The inventory includes the host CI and devenv union, both checkouts and their caches, sparse private state, the private output store, wrapper staging, and every candidate-owned root. The launcher rejects duplicate, nested, missing, or cross-filesystem roots.

The initial admission record requires no more than 10,000,000,000 allocated inventory bytes and at least 4,000,000,000 available filesystem bytes. These thresholds prove only the initial runner state. Every phase precheck and postcheck instead applies the separate 1,000,000,000-byte available-space safety floor. This split leaves usable build space while preserving an assumed fail-closed reserve.

For `BURL-O001`, the exact ordered phase IDs are `linux-flake-check`, `linux-build`, `linux-flake-install`, `linux-appimage-installed-probe`, `linux-flake-installed-probe`, `ubuntu-22.04-x86_64`, `ubuntu-24.04-x86_64`, `ubuntu-26.04-x86_64`, `debian-12-x86_64`, `debian-13-x86_64`, `linux-copy-candidate-outputs`, `linux-write-role-manifest`, `linux-compress-role-bundle`, and `linux-finalize-upload-inputs`. The wrapper records one initial admission observation plus an adjacent `precheck` and `postcheck` pair for each phase. An accepted transcript has exactly 29 records.

The final four phases run only after the teardown lock proves that no candidate process remains. They copy allowlisted candidate output, write the role manifest, compress the role bundle, and finalize the exact upload members. Every disk-backed temporary byte remains allocated until its phase postcheck. Ephemeral scratch uses tmpfs. This rule makes the maximum recorded allocation include each phase's disk peak.

The trusted wrapper writes the `linux-capacity.ndjson` newline-delimited JSON (NDJSON) file under the mode-`0700` root `$RUNNER_TEMP/burlmd-capacity-linux-x86_64-$ARTIFACT_NONCE`. `ARTIFACT_NONCE` is the authenticated 32-hex run nonce. The wrapper creates the root outside every checkout, candidate output, sparse store, and Bubblewrap mount. Before admission, it reserves 1,048,576 allocated bytes for the transcript with keep-size semantics. It rejects a failed reservation.

The wrapper tracks the expected phase, event, sequence, and monotonic time in parent memory. It compares each parsed append with that independent state. After the final postcheck, it validates every line against `contracts/linux-capacity-record.schema.json` version `1` and applies ADR-020's semantic invariants. The schema fixes every field name, type, enum, timestamp format, threshold, and filesystem-identity component.

The Linux `BURL-O001` candidate artifact contains exactly two members: the untrusted `ci-role-evidence.tar.zst` and the wrapper-owned `linux-capacity.ndjson`. The transcript isn't an `internalArtifacts` member. Candidate output can't reserve that name. The fresh seal validates both members independently and places the unchanged bytes in the attested sealed artifact. Aggregation consumes capacity only from that authenticated sealed member after every artifact, digest, attestation, origin, schema, ordering, and wrapper-state check succeeds.

After validation, aggregation writes four exact `cross-cutting` Spike-result measurements with unit `bytes` and `samples: 29`: inventory allocated peak, filesystem-used peak, minimum filesystem availability, and output-roots allocated peak. The raw contract fixes their machine names and derivations. Another name, candidate, unit, sample count, derivation, or unsealed source rejects the result.

The pinned upload action v7.0.1 uses `@actions/artifact` 6.2.0. Its default archive path pipes `archiver` into a bounded in-memory upload stream and creates no local ZIP file. The disk-capacity window therefore closes after upload-input finalization. Reverify this source on every action or archive-mode change. Add a bracketed upload phase if a replacement can allocate local archive bytes.

A missing or invalid record, a threshold breach, or a storage-exhaustion error fails the candidate job before upload. Because `seal` has an unchanged successful-only dependency, no sealed artifact, accepted aggregate, or schema-valid Spike result exists. The diagnostic job log isn't accepted evidence. A retry requires a fresh dispatch and run identity.

The standard `ubuntu-24.04` runner remains an assumed, fail-closed research choice. A local prototype reduced initial allocation from 15,382,421,504 bytes to 9,910,620,160 bytes. It didn't run the Linux packaging and finalization phases. Linux capacity evidence doesn't prove the separate macOS archive gates.

Under Nix 2.35.2, the local prototype's database subset export is 213,079 bytes. Its private state uses 8,785,920 allocated bytes. Its private store mount points and probe outputs use 2,076,672 allocated bytes. The prior 5,600,000,000-byte Nix Archive threshold isn't a capacity check and no longer applies. ADR-020 records the exact reproduction procedure and output facts.

The `seal` job has `needs: candidate` without an `always()` override. It starts in a fresh environment with no shared process namespace, workspace, or writable filesystem. Only the immutable untrusted candidate artifact crosses this boundary. For Linux `BURL-O001`, that artifact has the role bundle and protected capacity transcript. The seal validates the candidate runtime guards, expected identity, manifest, hashes, capacity schema and semantics, artifact identity, REST digest, and archive shape. It never executes candidate bytes.

Only `seal` receives `actions: read`, `contents: read`, `id-token: write`, and `attestations: write`. The pinned attest action uses `subject-path`, `push-to-registry: false`, and `create-storage-record: false`; this file mode needs no artifact-metadata authority. The seal creates deterministic `ci-sealed-role-evidence.tar.zst`. It contains only the unchanged candidate `ci-role-evidence.tar.zst`, except that Linux `BURL-O001` also contains the unchanged validated `linux-capacity.ndjson`. The seal attests and uploads that sealed bundle exactly once under the unique sealed artifact name with `overwrite: false` and `if-no-files-found: error`.
