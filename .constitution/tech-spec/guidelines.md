...
- `./scripts/managed-evidence.sh collect --source-ref SOURCE_REF --run-id RUN_ID --attempt RUN_ATTEMPT --source-head-sha SOURCE_HEAD_SHA --base-sha BASE_SHA --output REPORT_JSON` verifies an existing push-triggered run. It requires the named source ref to resolve remotely to `SOURCE_HEAD_SHA`. It also requires the run and attempt to use the `push` event, `.github/workflows/ci.yml`, that source ref, that source head, and the expected base.
...
Aggregation verifies each bundle and compares its captured identities with the independently supplied expected identity. It also requires the exact evidence-class sequence for each role. Linux can't claim macOS visual evidence, macOS can't claim Linux visual evidence, and macOS 15 can't claim performance or visual evidence. An operator commits the accepted or rejected aggregate as a later evidence-only commit whose parent is the tested source head and whose diff contains only `REPORT_JSON`. The caller ignores that exact report path, so the evidence commit doesn't become a validation run. The workflow never pushes repository content. Milestone review starts only from an accepted evidence commit. A draft-pull-request status alone is not evidence.
