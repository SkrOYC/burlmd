...
Contract fixtures run the create, check, record-current-only, teardown, remove, and verify-absent sequence across all seven sessions. They cover staging, session, and writable-bind leaves. Negative fixtures retain a removed prior leaf reference, add an undeclared leaf, escape through a symbolic link, cross a nested mount, change `st_dev`, add an authority after freeze, omit a frozen authority, and put 3,999,999,999 available bytes on a secondary stable device while the primary passes.

The trusted launcher creates one `logs/burl-m003-linux-closure-view.log` outside candidate-writable paths. The raw contract defines its exact UTF-8 and LF grammar and canonical golden fixture. The header records raw contract version `36`, the parent-only Bubblewrap path, version, executable digest, tool-closure digest, candidate-manifest identity, and source identity. Seven ordered session frames retain the manifest, source, namespace-flag, unique staging-root, and preflight digests. Each capacity row retains the stable authority identifier, that session's canonical current path, and its device. The trusted parser resolves the identifier against the frozen parent, kind, device, and mount declaration. The frame retains complete-vector, per-device start-space, and cleanup observations. The next session can't start until the preceding teardown lock is free and every ephemeral leaf from that session is absent. A later frame can't contain a removed earlier leaf. The literal `manifest-payload:` delimiter precedes exactly the candidate-manifest bytes. The manifest's final line feed is the log's final byte; no trailing data is allowed.

The Linux role manifest names this log in `roleEvidence.internalArtifacts` with its exact byte count and SHA-256. Fresh sealing parses the log and validates the golden grammar, exact payload, session order, session count, and every retained digest. It also enforces every identity, argument, preflight, start-space, teardown, and cleanup condition. The seal rejects a candidate manifest or `PATH` that exposes Bubblewrap. The accepted aggregate embeds the role manifest and binds the unchanged role and sealed bundles through `origin.roleBundleSha256` and `origin.sealedBundleSha256`. This retention uses role schema version `15` and aggregate schema version `19` without adding a field, evidence schema, or service artifact.
