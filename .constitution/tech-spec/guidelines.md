...
The workflow pins `cachix/install-nix-action` to commit `13d8dd58da0234aa297dedd986986ccb8e7f3e24`. That source pins its downloaded Nix version to `2.35.2`, but its `install_url` input can replace the download. Omit `install_url` and every environment or wrapper mapping that can supply `INPUT_INSTALL_URL`. Require `command -v nix` to fail before the action. Immediately afterward, require `nix --version` and `nix-store --version` to report exactly Nix `2.35.2` from the same installed profile. Run every closure reproduction with those observed executables. Static and runtime fixtures reject an override, preinstalled Nix, version mismatch, or executable mismatch.
