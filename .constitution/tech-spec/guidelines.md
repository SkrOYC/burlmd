...
The namespace exposes no host store root, Nix state, Nix database, daemon socket, or unlisted store path. `/nix/var`, `/nix/var/nix/db`, `/nix/var/nix/db/db.sqlite`, and `/nix/var/nix/daemon-socket/socket` must return `ENOENT`. The accepted runtime fixture first requires those host paths to exist with the applicable directory, regular-file, and socket types. This condition prevents a vacuous absence proof. A duplicate-aware source fixture permits only the owned-base `--ro-bind` to `/nix/store` and one same-path `--ro-bind` per manifest member. It rejects every other bind-family option whose source or destination is `/nix` or a descendant, including broad `/nix`, `/nix/store`, and `/nix/var` binds. One negative fixture omits a bind behind a correctly typed placeholder. Another retains all valid binds but seeds copied member bytes and private Nix state beneath them. The staging-shape check must reject the second fixture before Bubblewrap starts, so hidden copied state can't consume the private-store allocation while satisfying the view contract.

The trusted launcher creates `logs/burl-m003-linux-closure-view.log` outside candidate-writable paths. The log records raw contract version `36`, the exact manifest and its byte count and SHA-256, matching member and placeholder counts, zero staging payload entries and bytes, every member's expected and observed source identity, the complete null-terminated vector byte count, `ARG_MAX`, and the start-space observation. The candidate can't rewrite the trusted preflight prefix or the launcher-owned log.

The Linux role manifest names this log in `roleEvidence.internalArtifacts` with its exact byte count and SHA-256. Fresh sealing parses the log, verifies its fixed-order format and exact manifest payload, and enforces every count, hash, identity, argument, and start-space condition. The accepted aggregate embeds that role manifest and binds the unchanged role and sealed bundles through `origin.roleBundleSha256` and `origin.sealedBundleSha256`. This retention uses role schema version `15` and aggregate schema version `19` without adding a field or service artifact.

The launcher adds `--disable-userns` and `--assert-userns-disabled` to the single Bubblewrap invocation. Bubblewrap enters an internal nested user namespace to prevent the candidate from creating further user namespaces. Candidate assertions consume the mounted manifest and don't start a second Bubblewrap process. The no-network, private-PID, descriptor, teardown, and cleanup controls remain unchanged.
