...
Immediately before every namespace entry, the trusted launcher records each selected member's no-follow file type, `st_dev`, `st_ino`, containing mount device, and computed mount root. Base snapshots must match each other. Integration snapshots must match each other, and their base-member rows must equal the base snapshot. The launcher writes the selected snapshot to `/contract/locked-nix-closure.sources` and exposes it read-only to the preflight. It creates one fresh staging leaf under the frozen stable parent. The leaf contains one source-distinct empty placeholder per selected member. Bubblewrap mounts the base read-only at `/nix/store`, then mounts each selected member at its canonical path.
...
On every integration outcome, the supervisor terminates the candidate group and Sway, waits for Sway, and verifies that its PID no longer exists. With no hostile process alive, it writes a canonical cleanup frame. The parent validates and fsyncs the matching retained session frame before acknowledging namespace exit. Early Sway failure, hostile candidate exit, timeout, socket disruption, interruption, graceful reap, and forced reap all have fixtures.
