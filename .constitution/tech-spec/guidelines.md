...
TechSpec v2.1.29 (translated from reviewed v1.8.7-provisional) permits research code only under `.constitution/prototypes/`. The Epic G M0 production exceptions remain unchanged. `BURL-M015` and `BURL-M003` may write production code only for their reproducibility and validation bootstrap. During the PR #37 correction, the live raw-version-39 Stage 4 adaptation is `required`.

Tasks v3.2.13 at source-branch commit `46bc8ee96b5d2d58d27ac33991c720d2eee76511` contains the preceding reviewed adaptation. It doesn't quote the exact diagnostic map and prefix from this correction. A separate Stage 4 correction in PR #37 must quote those values. It must also replace `BURL-O001`'s acknowledgement-only stop with the whole-PR merge gate. The correction updates only the current unmerged Tasks v3.2.13 changelog entry's upstream TechSpec pointer to v2.1.29. Older Tasks history remains unchanged.

A final Stage 3 correction must cite the corrected Tasks source-branch commit and restore the adaptation to `satisfied`. The source-branch SHA won't be the commit's rebase-merged SHA. `BURL-O001` remains a maintained stop and isn't authorized.
