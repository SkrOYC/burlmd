...
On every integration outcome, the supervisor terminates the candidate group and Sway, waits for Sway, and verifies that its PID no longer exists. It also requires no `swaybg` process or output. With no hostile process alive, it writes a canonical cleanup frame. The parent validates and fsyncs the matching retained session frame before acknowledging namespace exit. Early Sway failure, hostile candidate exit, timeout, socket disruption, interruption, graceful reap, forced reap, and injected `swaybg` process and output canaries all have fixtures.
...
This closure view authorizes only `BURL-M003`. The Stage 4 adaptation record lists `BURL-M003` under `affected_tickets` and `BURL-O001` under `maintained_stop_tickets`. The latter preserves the Stage 3 stop without authorizing `BURL-O001` implementation.
