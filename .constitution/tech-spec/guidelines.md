...
Immediately after the handshake succeeds and before any candidate, Sway, or `swaymsg` process starts, the trusted supervisor runs the pinned iproute2 `7.0.0` command `ip link set dev lo up`. It then runs `ip -o link show up dev lo` and requires status zero plus one nonempty line whose flags include `UP`. The candidate loopback fixture must bind, listen, connect, and exchange bytes over `127.0.0.1`. No other interface, route, or external network access is allowed.
