...
In the trusted parent, resolve `bwrap` to `locked_bubblewrap_store_member/bin/bwrap`. Raw contract version `36` pins the exact x86-64 Linux store member. Require that member in the manifest. Run the exact executable with only `--version`. Require the output `bubblewrap 0.11.2` plus one line feed, empty standard error, and status zero. Use the same path for the sole candidate Bubblewrap invocation. The trusted preflight and candidate don't execute Bubblewrap.

The trusted preflight writes a length-prefixed record to a parent-owned pipe before candidate execution. It writes `preflight-bytes=<canonical-decimal>` plus one line feed, the declared number of bytes, and then closes the pipe. The parent requires the exact length, end of file, and a successful preflight. It validates the body before it acknowledges candidate start. The wrapper closes both handshake descriptors before the candidate command.

Before the first candidate command, create and canonicalize the complete BURL-M003 writable-root inventory. It includes the source checkout, disposable workspace, output and result roots, dependency homes and caches, build and generated outputs, temporary directories, candidate home and configuration, closure staging, session storage, private Sway runtime, every writable-bind source, and every writable environment path. No later step can add another writable root. Group the roots by distinct `st_dev`. Immediately before namespace entry, require `statvfs.f_bavail * statvfs.f_frsize` to be at least 4,000,000,000 bytes for each distinct containing filesystem. Reject an omitted root or device, inconsistent observations, and a secondary device below the floor even when the primary device passes.

The trusted launcher creates `logs/burl-m003-linux-closure-view.log` outside candidate-writable paths. The raw contract defines its exact UTF-8 and LF grammar and a canonical golden fixture. The log records raw contract version `36`, the exact Bubblewrap path and version, the manifest identity, staging observations, complete-vector size, `ARG_MAX`, every root-to-device mapping, one available-byte observation per distinct device, and every member's source identity. Its literal `manifest-payload:` delimiter precedes exactly the declared manifest bytes. The manifest's final line feed is the log's final byte; no trailing data is allowed.

The Linux role manifest names this log in `roleEvidence.internalArtifacts` with its exact byte count and SHA-256. Fresh sealing parses the log, validates the golden grammar and exact payload, and enforces every count, hash, identity, argument, and per-device start-space condition. The accepted aggregate embeds that role manifest and binds the unchanged role and sealed bundles through `origin.roleBundleSha256` and `origin.sealedBundleSha256`. This retention uses role schema version `15` and aggregate schema version `19` without adding a field or service artifact.

The launcher adds `--disable-userns` and `--assert-userns-disabled` to the one candidate Bubblewrap invocation. Bubblewrap enters an internal nested user namespace to prevent the candidate from creating further user namespaces. Candidate assertions check the resulting namespace, mount, process, descriptor, and network properties. They don't execute Bubblewrap. The no-network, private-PID, descriptor, teardown, and cleanup controls remain unchanged.

Before `exec`, count the complete null-terminated `env -i` environment, Bubblewrap arguments, closure binds, `env` symlink arguments, and candidate command. Reject a value at or above half of `getconf ARG_MAX`. The per-device space check is a start guard only. It isn't a phase transcript, phase-peak measurement, or packaging-capacity claim.
