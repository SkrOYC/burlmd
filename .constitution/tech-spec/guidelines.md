...
Before the first candidate session, create and canonicalize the complete BURL-M003 writable-root inventory. It includes the source checkout, disposable workspace, output and result roots, dependency homes and caches, build and generated outputs, temporary directories, candidate home and configuration, closure staging, session storage, private Sway runtime, every writable-bind source, and every writable environment path. No later step can add another writable root. Immediately before every namespace entry, group the roots by distinct `st_dev`. Require `statvfs.f_bavail * statvfs.f_frsize` to be at least 4,000,000,000 bytes for each distinct containing filesystem. Reject an omitted root or device, inconsistent observations, and a secondary device below the floor even when the primary device passes.

The trusted launcher creates one `logs/burl-m003-linux-closure-view.log` outside candidate-writable paths. The raw contract defines its exact UTF-8 and LF grammar and canonical golden fixture. The header records raw contract version `36`, the parent-only Bubblewrap path, version, executable digest, tool-closure digest, candidate-manifest identity, and source identity. Seven ordered session frames retain the manifest, source, namespace-flag, unique staging-root, and preflight digests. Each frame also retains complete-vector, root-to-device, per-device start-space, and cleanup observations. The next session can't start until the preceding teardown lock is free and its staging root is absent. The literal `manifest-payload:` delimiter precedes exactly the candidate-manifest bytes. The manifest's final line feed is the log's final byte; no trailing data is allowed.
...
Before each `exec`, count the complete null-terminated `env -i` environment, Bubblewrap arguments, closure binds, `env` symlink arguments, and that session's candidate command. Reject a value at or above half of `getconf ARG_MAX`. Each per-device space check is a start guard only. It isn't a phase transcript, phase-peak measurement, or packaging-capacity claim.
