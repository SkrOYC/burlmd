...
After trusted Nix and tool preparation, the trusted `ubuntu-24.04` wrapper runs the same loopback sequence in a minimal pinned Bubblewrap probe. It does this before tested-source dependency execution or any candidate process. If the first probe succeeds, don't change host policy. If it fails, continue only when the hosted-runner guards pass, `/proc/sys/kernel/apparmor_restrict_unprivileged_userns` contains exactly `1`, `/usr/bin/sudo` and `/usr/sbin/sysctl` pass the raw contract's ownership and mode checks, and noninteractive sudo is available. Install `EXIT`, `HUP`, `INT`, `QUIT`, and `TERM` restoration handlers before running `/usr/bin/sudo --non-interactive -- /usr/sbin/sysctl -w kernel.apparmor_restrict_unprivileged_userns=0`. Require the proc value to equal `0`, then rerun the exact probe. Reject any failed precondition, write, value check, or second probe.

On success, catchable failure, timeout, and every handled signal, restore the saved value with the same exact sudo and sysctl executables. Require the final proc value to match before creating or uploading the bundle. Restoration failure blocks upload. `SIGKILL`, abrupt runner loss, and host failure can't run the handler and can't produce an accepted upload. The ephemeral hosted runner boundary is the residual limit on the temporary host-wide change. Candidate code receives no sudo executable, host sysctl mount, host sysctl descriptor, or workflow step with policy authority. Retain the hosted image, kernel, initial and final values, probe results, and restoration result. Local systems without this proc entry can validate the no-override probe and state-machine fixtures, but only the managed run can establish hosted behavior.
