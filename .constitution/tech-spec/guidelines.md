...
The Linux candidate uses an exact closure view instead of copying its Nix closure. The trusted launcher derives a sorted manifest with `nix-store -qR` from the ticket's declared tool roots. It verifies that every direct Nix reference remains in that manifest. Each manifest member must be an existing, canonical, nonsymlink `/nix/store` entry. Internal links remain intact inside the mounted entry.
...
Capacity checks use allocated filesystem bytes rather than Nix Archive (NAR) size. Immediately before build output, the launcher measures each unique allocated root on the containing filesystem. The inventory includes the host CI and devenv union, checkout and dependency caches, signer checkout, sparse private state, private output store, and other candidate roots. The sum must not exceed 10,000,000,000 bytes. It must also leave at least 4,000,000,000 bytes against the standard runner's 14,000,000,000-byte profile. The launcher checks available space again before each build phase. It fails closed on either boundary.

The capacity correction preserves the standard `ubuntu-24.04` role. The measured prebuild allocation falls from 15,382,421,504 bytes to 9,910,620,160 bytes when the copied closure is replaced by sparse state. Under Nix 2.35.2, the prototype's database subset export is 213,079 bytes. Its private state uses 8,785,920 allocated bytes, and its private store mount points and probe outputs use 2,076,672 bytes. The prior 5,600,000,000-byte Nix Archive (NAR) threshold is not a capacity check and no longer applies.
