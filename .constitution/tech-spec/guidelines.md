...
verification_commands:
  ...
  - name: format
    ...
    owner: BURL-M003
  - name: format
    label: temporary Stage 4 compatibility command quoted by BURL-M003 before OD-11 adaptation
...
The seal attests the manifest and copies the action's `bundle-path` output to `compatibility-stage-attestation.sigstore.json`. It uploads those two files and the two producer members under `managed-evidence-authenticated-stage-macos-26-arm64-for-macos-15-arm64-{artifactNonce}`. The pinned upload action returns `artifact-id` and the bare `artifact-digest`. The seal queries that exact artifact through REST API version `2026-03-10`. It requires the canonical REST `digest` to equal `sha256:` plus the bare action digest. It also records `created_at`, `expires_at`, and `expired: false`.

Sealing receipt version `2` stores the resulting `producerLineage`. The lineage binds the exact stage name, service artifact ID, both digest forms, artifact lifetime, manifest, attestation bundle digest, subject, signer, source, run, attempt, and producing seal locator. The macOS 26 role exports these facts and the exact producer-receipt artifact identity to the macOS 15 role as trusted reusable-workflow inputs.

The macOS 15 trusted wrapper downloads the stage and producer receipt by immutable `artifact-ids`. It sets `digest-mismatch: error` explicitly and does not use `name`. During acquisition, it obtains a fresh `trusted_root.jsonl`. The wrapper then verifies the stage-manifest and producer-receipt attestations with local `--bundle` files and `--custom-trusted-root`. It constrains the repository, signer workflow, signer digest, source digest, and self-hosted policy. The verification output must match the expected subject, workflow, source, run, attempt, role, and check-run facts.

After verification, the wrapper hashes its verification output and trusted root into `consumerBinding`. It then removes credentials, authentication descriptors, temporary credential files, and ambient configuration. Only the two verified producer members enter a fresh wrapper-owned read-only input root. Candidate commands can't access the stage manifest, attestation bundles, receipt, trusted root, acquisition output, credentials, or writable producer bytes.

After candidate execution, the trusted wrapper writes `producerLineage` and `consumerBinding` into the untrusted role manifest. The macOS 15 seal reconstructs the trusted wrapper facts independently and requires exact equality. It writes the unchanged lineage and consumer binding into its version `2` receipt. Final aggregation compares both receipts, the macOS 15 manifest, current stage REST state, and both verified seal origins.
...
The client enumerates the exact run's artifacts. For every expected, candidate, sealed, receipt, and compatibility-stage artifact, the upload action output is bare lowercase 64-hex. The REST `digest` and aggregate `artifactDigest` use `sha256:<hex>`. The REST value must equal `sha256:` plus `uploadActionDigest`. A prefix, case, or byte mismatch rejects the artifact. The client then validates the receipt, sealed bundle, manifest, and every internal member before using a result.
