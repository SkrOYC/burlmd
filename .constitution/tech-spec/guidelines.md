...
For `BURL-M003`, ADR-020 replaces the copied private closure with two exact read-only views. Raw contract version `37` derives the base closure from the required-tool, runtime-link, and M003 profile roots. It includes `jq`, OpenSSL, Mesa, Diffutils `3.12`, and Procps `4.0.6`. The integration closure is the sorted union of the base and exact unwrapped Sway and `swaymsg` closure. Bubblewrap remains outside both views.
...
Every invocation creates only its declared staging, session, contract, and writable-bind leaves under their frozen stable parents. Only integration invocations create and bind an `xdg-runtime` leaf. Every invocation includes `--unshare-all`, `--unshare-user`, `--uid 0`, `--gid 0`, `--unshare-net`, `--disable-userns`, `--assert-userns-disabled`, `--cap-add CAP_NET_ADMIN`, `--die-with-parent`, and `--new-session`. The trusted preflight and candidate don't execute another Bubblewrap process. Every class omits Bubblewrap. Base sessions also omit Sway and `swaymsg`; integration sessions contain their exact unwrapped members.
...
The parent requires the exact length and end of file before validation. After successful validation, it closes the record read end, writes exactly ASCII `G`, and closes the acknowledgement write end. The wrapper requires `G` followed by end of file and closes descriptor 4. Immediately before it replaces itself with the candidate command, it requires exactly descriptors 0, 1, and 2. Each standard stream must retain the `fstat` file type, `st_dev`, `st_ino`, decoded `/proc/self/fd` target, and access mode recorded before pipe creation. Any leak, reversed end, early acknowledgement, wrong byte, missing end of file, changed standard stream, or ordering failure stops the session before candidate execution.
...
The launcher serializes every complete Bubblewrap argv as NUL-delimited UTF-8, hashes it, and executes that same in-memory vector. The digest covers the executable, every option, every directory and bind source and destination, the supervisor, and the session command. Static fixtures reject every addition, omission, reordering, duplication, or substitution. Extra capabilities, contract-mount mutations, runtime-mount mutations, and host-root alias binds are explicit cases. The integration golden is 783 bytes with SHA-256 `a4e021dfad7430fda7a0143646a7f8971d9a703930b8a9279d8da5575b4e4ad4`. A byte count, set comparison, fixed-flag digest, successful parse, or successful execution isn't sufficient.

The trusted launcher creates one `logs/burl-m003-linux-closure-view.log` outside candidate-writable paths. Raw contract version `37` defines the version 2 golden grammar. It retains both manifest payloads, all frozen authorities and current paths, exact session classes, each complete-argv byte count and SHA-256, preflight evidence, capacity observations, and integration cleanup results. The golden authority set has 16 rows and SHA-256 `84c452be4629b446fbba93e15c771dab5f79c499d1d620968e1736ce583def52`. Fresh sealing reconstructs every argv, including each integration runtime bind, from trusted contract data and retained authority and current-path fields. It requires exact equality before it authenticates the bundle.
...
Bubblewrap enters an internal nested user namespace to prevent the candidate from creating further user namespaces. Candidate assertions check the resulting namespace, mount, process, descriptor, and network properties. They don't execute Bubblewrap. Every session preserves the no-network, private-PID, descriptor, teardown, survivor-rejection, and cleanup controls. Nested and candidate-started Bubblewrap remain forbidden.
