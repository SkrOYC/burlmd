...
Managed-Spike collection uses two execution phases. The merged and reviewed `CI-M003` client controls both phases; candidate branches can't replace that client or its trusted coordinator-step contract.
...
Before coordinator execution, the client closes authentication clients and inherited descriptors, unsets `GH_TOKEN`, `GITHUB_TOKEN`, Actions OIDC variables, `SSH_AUTH_SOCK`, `GIT_ASKPASS`, package-registry tokens, and common cloud credential variables, and discards every temporary authentication file. It then launches Bubblewrap with all namespaces unshared, `--unshare-net`, `--new-session`, and `--die-with-parent`. Bubblewrap `0.11.2` has no `--close-fds` option; its pinned source closes every descriptor except standard input, output, error, and descriptors explicitly required for sandbox setup before it executes the child. The launcher passes no optional descriptor, and the fixture verifies the child can't observe an inherited canary descriptor. Failure to establish any namespace or mount rejects the aggregate.
...
Credential-isolation fixtures inject unique canaries into GitHub, Git, SSH-agent, package-registry, and cloud environment variables and standard configuration paths. A probe coordinator dumps its environment, checks the standard paths, attempts to locate the GitHub CLI and Git, tests inherited descriptors, and attempts a network connection. No canary, credential path, client, extra descriptor, or network route may be visible. The same sandbox must still run a representative coordinator successfully and produce a valid result from read-only inputs. The aggregate records the executable and closure identities plus every isolation check.
