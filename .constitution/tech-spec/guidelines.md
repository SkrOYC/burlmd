...
Before the first candidate session, create and canonicalize every stable BURL-M003 writable parent. Freeze the capacity authority set from those parents and devices, not from ephemeral leaves. The set covers every source in the raw contract's dynamic mount plan, writable environment paths, staging, sessions, output, caches, builds, generated output, homes, and per-session runtime storage. Each capacity slot declares its complete identity and one permitted leaf per session. No later step can add a parent, mount, device, slot, leaf, writable path, or argv source outside the set.
...
Contract fixtures run the create, check, record-current-only, teardown, remove, and verify-absent sequence across all seven sessions. They cover staging, session, and writable-bind leaves. Negative fixtures retain a removed prior leaf reference, add an undeclared leaf, escape through a symbolic link, cross a changed or nested mount with the same device, change `st_dev`, substitute an authority, use the wrong kind, parent, ownership, or mode, add an authority after freeze, omit a frozen authority, and put 3,999,999,999 available bytes on a secondary stable device while the primary passes.

The launcher serializes every complete Bubblewrap argv as NUL-delimited UTF-8, hashes it, and executes that same in-memory vector. The digest covers the executable, every option, every directory and bind source and destination, the supervisor, and the session command. Static fixtures reject every addition, omission, reordering, duplication, or substitution. Extra capabilities, contract-mount mutations, and host-root alias binds are explicit cases. The updated golden is 533 bytes with SHA-256 `31df65a44206be9976b25562dab9fcf675dc4500a268c54171e79c60253fa1e4`. A byte count, set comparison, fixed-flag digest, successful parse, or successful execution isn't sufficient.

The trusted launcher creates one `logs/burl-m003-linux-closure-view.log` outside candidate-writable paths. Raw contract version `37` defines the version 2 golden grammar. It retains both manifest payloads, all frozen authorities and current paths, exact session classes, each complete-argv byte count and SHA-256, preflight evidence, capacity observations, and integration cleanup results. Fresh sealing reconstructs every argv from trusted contract data and retained authority/current-path fields. It requires exact equality before it authenticates the bundle.

The Linux role manifest names this log in `roleEvidence.internalArtifacts` with its exact byte count and SHA-256. Fresh sealing parses the log and validates the golden grammar, both payloads, session order, authority rows, contract content, argv digests, candidate launch environment, Sway configuration hash, absence of `swaybg`, and cleanup. It rejects class drift, an exposed Bubblewrap member, a compositor member in a base view, an incomplete integration view, or ambiguous current-path resolution. The accepted aggregate embeds the role manifest and binds the unchanged role and sealed bundles through `origin.roleBundleSha256` and `origin.sealedBundleSha256`. This retention uses role schema version `15` and aggregate schema version `19` without adding a field, evidence schema, or service artifact.
