...
For every session, a fresh trusted preflight writes a length-prefixed record to a parent-owned pipe before candidate execution. It writes `preflight-bytes=<canonical-decimal>` plus one line feed, the declared number of bytes, and then closes the pipe. The parent requires the exact length, end of file, and a successful preflight. It validates the body before it acknowledges candidate start. The wrapper closes both handshake descriptors before the candidate command.
