...
The trusted caller's `receipt_digests` job needs `expected` and all three role-calling jobs. It runs on `ubuntu-24.04`, receives only `contents: read`, and downloads no candidate artifact. The job passes the six role outputs to `scripts/write-receipt-digest-observation.sh`. The helper writes `receipt-upload-digests.json` against `ci-evidence.schema.json#/$defs/receiptDigestTransport`. The record binds exactly one receipt ID, name, role, and bare upload digest for each role to the expected identity, repository, run, attempt, signer, and artifact nonce. The job uploads that file once as `managed-evidence-receipt-digests-{artifactNonce}` with `overwrite: false` and `if-no-files-found: error`.

After completion, `collect` downloads the unique caller transport and matches every transported receipt ID and name to the exact REST artifact object. It requires each REST `digest` to equal `sha256:` plus the transported `uploadActionDigest`. The aggregate retains both values in `origin.sealingReceiptArtifact`. A missing or malformed transport, identity mismatch, or digest mismatch rejects the run. The embedded sealing receipt remains an honest record of pre-upload facts.
