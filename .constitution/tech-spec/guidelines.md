...
verification_commands:
  ...
  - name: test
    ...
    exists: true
  - name: format
    label: canonical raw-39 verification for BURL-M003
...
TechSpec v2.1.29 (translated from reviewed v1.8.7-provisional) permits research code only under `.constitution/prototypes/`. The Epic G M0 production exceptions remain unchanged. `BURL-M015` and `BURL-M003` may write production code only for their reproducibility and validation bootstrap. The corrected raw-version-39 Stage 4 handoff is `satisfied`; this handoff is complete, not the implementation.

Tasks v3.2.13 includes the source-branch Stage 4 correction in PR #37 at commit `089786c07e904fd8384aa477617358d2b1d1fc66`. It quotes the exact diagnostic map, prefix, trailing-space semantics, and required mutation fixtures from this correction. It also replaces `BURL-O001`'s acknowledgement-only stop with the whole-PR merge gate. The correction updates only the current unmerged Tasks v3.2.13 changelog entry's upstream TechSpec pointer to v2.1.29. The complete PR #37 review must cover this correction before merge. Older Tasks history remains unchanged.

A final Stage 3 acknowledgement cites that source-branch correction and records the corrected handoff as complete. The source-branch SHA won't be the commit's rebase-merged SHA. `BURL-O001` remains independently blocked, a maintained stop, and isn't authorized.

The complete reviewed PR #37 must merge before the recovery implementation starts. The lead must then record the source-to-landed Tasks SHA and final landed documentation tip on PR #37. Complete Epic M, including the reviewed `BURL-M003` recovery implementation, fresh accepted managed run, exact three-file evidence pull request, epic closeout, and reconciliation. Then pause before Epic H or Epic G implementation until the user explicitly resumes that work. Every other production ticket remains blocked by its own decision evidence and matching Stage 3 and Stage 4 adaptation. Except for these contract-scoped exceptions, production directories (`lib/`, `rust/`, `linux/`, and `macos/`) are read-only inputs to this research wave.
...
For `BURL-M003`, ADR-020 replaces the copied private closure with two exact read-only views. Raw contract version `39` derives the base closure from the required-tool, runtime-link, and M003 profile roots. It includes `jq`, OpenSSL, Mesa, Diffutils `3.12`, Procps `4.0.6`, and iproute2 `7.0.0`. The integration closure is the sorted union of the base and exact unwrapped Sway and `swaymsg` closure. Bubblewrap remains outside both views.
