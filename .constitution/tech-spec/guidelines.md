...
TechSpec v2.1.17 (translated from reviewed v1.8.7-provisional) permits research code only under `.constitution/prototypes/`. The Epic G M0 production exceptions remain unchanged. `BURL-M015` and `BURL-M003` may write production code only for their reproducibility and validation bootstrap. Stage 4 must adapt `BURL-M003` to raw contract version `38` and retain the `BURL-O001` Stage 3 stop. Every other production ticket remains blocked by its own decision evidence and matching Stage 3 and Stage 4 adaptation. Except for these contract-scoped exceptions, production directories (`lib/`, `rust/`, `linux/`, and `macos/`) are read-only inputs to this research wave.
...
After trusted Nix and tool preparation, the trusted `ubuntu-24.04` wrapper runs the same loopback sequence in a minimal pinned Bubblewrap probe. It does this before tested-source dependency execution or any candidate process. If the first probe succeeds, don't change host policy. If it fails, continue only when the hosted-runner guards pass, `/proc/sys/kernel/apparmor_restrict_unprivileged_userns` contains exactly `1`, `/usr/bin/sudo` and `/usr/sbin/sysctl` pass the raw contract's ownership and mode checks, and noninteractive sudo is available. Install the unconditional exit-and-signal restoration handler before running `/usr/bin/sudo --non-interactive -- /usr/sbin/sysctl -w kernel.apparmor_restrict_unprivileged_userns=0`. Require the proc value to equal `0`, then rerun the exact probe. Reject any failed precondition, write, value check, or second probe.

On success, failure, timeout, and every handled signal, restore the saved value with the same exact sudo and sysctl executables. Require the final proc value to match before creating or uploading the bundle. Restoration failure blocks upload. Candidate code receives no sudo executable, host sysctl mount, host sysctl descriptor, or workflow step with policy authority. Retain the hosted image, kernel, initial and final values, probe results, and restoration result. Local systems without this proc entry can validate the no-override probe and state-machine fixtures, but only the managed run can establish hosted behavior.

Before the first candidate session, create and canonicalize every stable BURL-M003 writable parent. Freeze the capacity authority set from those parents and devices, not from ephemeral leaves. The set covers every source in the raw contract's dynamic mount plan, writable environment paths, staging, sessions, output, caches, builds, generated output, and homes. It also covers exactly two `xdg-runtime` declarations, one per integration session. Base sessions have no runtime declaration. Each capacity slot declares its complete identity and permitted leaf. No later step can add a parent, mount, device, slot, leaf, writable path, or argv source outside the set.
...
This closure view authorizes only `BURL-M003`. It doesn't authorize or solve `BURL-O001` Nix packaging isolation or capacity. Route that work to a later Stage 3 decision before `BURL-O001` implementation.
