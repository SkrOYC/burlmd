...
verification_commands:
  ...
  - name: test
    ...
    exists: true
  - name: format
    label: canonical OD-11 verification for BURL-M003
...
For `BURL-M003`, ADR-020 replaces the copied private closure with two exact read-only views. Raw contract version `38` derives the base closure from the required-tool, runtime-link, and M003 profile roots. It includes `jq`, OpenSSL, Mesa, Diffutils `3.12`, Procps `4.0.6`, and iproute2 `7.0.0`. The integration closure is the sorted union of the base and exact unwrapped Sway and `swaymsg` closure. Bubblewrap remains outside both views.
...
After trusted Nix and tool preparation, the trusted `ubuntu-24.04` wrapper runs the same loopback sequence in the exact minimal pinned Bubblewrap probe. It does this before tested-source dependency execution or any candidate process. Status zero is a required hosted prerequisite. Any nonzero status, signal, timeout, namespace failure, loopback failure, or malformed result blocks candidate execution, bundle creation, and upload. The run has no accepted evidence.
...
This closure view authorizes only `BURL-M003`. Tasks v3.2.11 added acceptance requirements for the retained-descriptor teardown verification and post-reap cleanup-descriptor transition within that ticket's existing launcher, helper, and test paths. For each candidate namespace and session payload, the parent launches only Bubblewrap. The existing trusted preparation, identity checks, and pinned post-session `flock` verifier remain required and don't form a general process allowlist. The satisfied adaptation record lists `BURL-M003` under `affected_tickets` and `BURL-O001` under `maintained_stop_tickets`. The latter preserves the Stage 3 stop without authorizing `BURL-O001` implementation.
