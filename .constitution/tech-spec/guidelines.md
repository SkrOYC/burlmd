...
Candidate output is untrusted. On Linux, the trusted launcher starts candidate commands with `env -i` in a Bubblewrap `0.11.2` private PID namespace. It must prove that a double-fork probe does not survive the teardown lock before upload. Any failed command, required Linux teardown, bundle creation, or upload fails the candidate job. On hosted macOS, the launcher terminates and reaps known test processes and records cleanup failure. That bounded cleanup does not claim zero survivors or lifecycle containment for arbitrary candidate code. The candidate job then creates `ci-role-evidence.json` and the complete deterministic `ci-role-evidence.tar.zst` as untrusted output. The trusted workflow wrapper invokes the pinned upload action with `overwrite: false` and `if-no-files-found: error` for the exact unique candidate artifact name. It exports the action's artifact ID and bare lowercase 64-hex `artifact-digest` as trusted job outputs.
...
A candidate attempt to reserve a sealed or receipt name makes the immutable upload fail, but candidate processes lack the runtime artifact credentials needed to upload either artifact. The fresh environment keeps candidate bytes away from OIDC and attestation steps. It doesn't make candidate output trusted or claim universal macOS process containment. Bubblewrap remains the Linux candidate-containment and post-download coordinator boundary.
