...
TechSpec v2.1.13 (translated from reviewed v1.8.7-provisional) permits research code only under `.constitution/prototypes/`. The Epic G M0 production exceptions remain unchanged. `BURL-M015` and `BURL-M003` may write production code only for their reproducibility and validation bootstrap. Stage 4 must adapt `BURL-M003` to raw contract version `36` and add or retain the `BURL-O001` Stage 3 stop condition. Every other production ticket remains blocked by its own decision evidence and a matching Stage 3 and Stage 4 adaptation. Except for these contract-scoped exceptions, production directories (`lib/`, `rust/`, `linux/`, and `macos/`) are read-only inputs to this research wave.
...
For `BURL-M003`, ADR-020 replaces the copied private closure with an exact read-only closure view. The launcher derives the complete candidate manifest from the required-tool, runtime-link, and M003 profile roots in raw contract version `36`. The candidate roots include `jq`, OpenSSL, Mesa, Diffutils `3.12`, and Procps `4.0.6`. `BURL-M003` must add `pkgs.diffutils` and Linux-only `pkgs.procps` to `devenv.nix`. The launcher requires the locked `cmp` and `ps` store paths from the raw contract. It rejects root-resolution results under ambient `/usr`, `/run/current-system`, or any other non-store path. Bubblewrap, Sway, `swaymsg`, and members reached only from their parent closures are excluded.

Two pinned-Nix reproductions produced the same candidate closure: 488 paths, a 30,717-byte manifest, SHA-256 `127043afe260d7756ee6cbda03e39a5bfceb4f7f79ae3a5be1595e447ce15e64`, 6,184,635,112 summed Nix archive (NAR) bytes, and 66,314 bind-only argument bytes. The Bubblewrap parent closure remains eight paths, a 480-byte manifest, SHA-256 `398d11c9cd9249369cbb18d36661014eafef5ac18ff7adeb076c2c51ef0141fd`, 40,679,016 summed NAR bytes, and 1,040 bind-only argument bytes. The separate compositor parent closure has 198 paths, a 12,024-byte manifest, SHA-256 `2989c9144aedb0861d16e440a309e4564679009f4785525033b2a8e1ecf97f3d`, 649,910,672 summed NAR bytes, and 26,028 bind-only argument bytes.

Immediately before every namespace entry, the trusted launcher records each member's no-follow file type, `st_dev`, `st_ino`, containing mount device, and computed mount root. It requires every path-sorted snapshot to be byte-identical, writes the snapshot to `/contract/locked-nix-closure.sources`, and exposes the file read-only to that session's preflight. The launcher creates a fresh staging leaf only under its frozen stable parent. The leaf's only child is the store base. The base contains exactly one source-distinct placeholder of the correct type per manifest member. Directory placeholders are empty. Regular-file placeholders have zero length and zero allocated blocks. Bubblewrap mounts the base read-only at `/nix/store`, then mounts each manifest member read-only at its canonical path.
...
In the trusted parent, keep Bubblewrap in the separate eight-path tool closure at `/contract/trusted-parent-bubblewrap.manifest`. Raw contract version `36` pins its exact x86-64 Linux store member, executable, version output, executable SHA-256, and tool-closure manifest SHA-256. Validate those identities before the first session and revalidate both digests before every later session. Don't expose the tool closure through a candidate mount or `PATH`.

Keep Sway and `swaymsg` in the separate parent compositor closure at `/contract/trusted-parent-compositor.manifest`. Validate both exact executable paths, version outputs, executable SHA-256 values, and the closure digest. The parent starts and owns Sway and configures it with only the pinned `swaymsg`. Candidate sessions receive only the exact live Wayland socket at `/candidate/xdg/runtime/$WAYLAND_DISPLAY`. They receive no parent runtime directory, Sway IPC socket, compositor store member, executable, or `PATH` entry. Candidate traces contain no Sway or `swaymsg` process.
...
Before the first candidate session, create and canonicalize every stable BURL-M003 writable parent. Freeze the capacity authority set from those parents and their devices, not from ephemeral leaves. The set covers the source checkout, disposable workspace, output and result storage, dependency homes and caches, build and generated output, temporary storage, candidate home and configuration, closure staging, session storage, private Sway runtime, every writable-bind source, and every writable environment path. Each capacity slot declares its stable identifier, kind, canonical parent, required UID and GID, exact mode, no-follow directory type, `st_dev`, decoded containing-mount ID, major:minor, root, mount point, and one permitted leaf for every session. No later step can add a parent, mount, device, capacity slot, leaf declaration, or writable path outside this set.
...
The trusted launcher creates one `logs/burl-m003-linux-closure-view.log` outside candidate-writable paths. The raw contract defines its exact UTF-8 and LF grammar and canonical golden fixture. The header records raw contract version `36`, both parent closure identities, candidate-manifest identity, source identity, and capacity-authority digest. Before the session frames, canonical authority rows retain every frozen field and permitted leaf. Each session row binds that digest. Each current-path row must resolve uniquely by ordinal, session ID, and stable identifier. Seven ordered session frames retain the manifest, source, authority, namespace-flag, unique staging-root, and preflight digests. The frame also retains complete-vector, per-device start-space, and cleanup observations. The next session can't start until the preceding teardown lock is free and every ephemeral leaf from that session is absent. A later frame can't contain a removed earlier leaf. The literal `manifest-payload:` delimiter precedes exactly the candidate-manifest bytes. The manifest's final line feed is the log's final byte; no trailing data is allowed.

The Linux role manifest names this log in `roleEvidence.internalArtifacts` with its exact byte count and SHA-256. Fresh sealing parses the log and validates the golden grammar, exact payload, session order, session count, authority rows, and every retained digest. It enforces every identity, argument, preflight, start-space, teardown, and cleanup condition. The seal rejects a changed or same-device nested mount, wrong kind, parent, ownership, or mode, undeclared leaf, authority substitution, or ambiguous current-path resolution. It also rejects a candidate manifest or `PATH` that exposes Bubblewrap, Sway, or `swaymsg`. The accepted aggregate embeds the role manifest and binds the unchanged role and sealed bundles through `origin.roleBundleSha256` and `origin.sealedBundleSha256`. This retention uses role schema version `15` and aggregate schema version `19` without adding a field, evidence schema, or service artifact.
...
Before each `exec`, count the complete null-terminated `env -i` environment, Bubblewrap arguments, closure binds, `env` symlink arguments, and that session's candidate command. Reject a value at or above half of `getconf ARG_MAX`. Remove every ephemeral leaf after success, failure, interruption, or timeout. Each per-device space check is a start guard only. It isn't a phase transcript, phase-peak measurement, or packaging-capacity claim.
