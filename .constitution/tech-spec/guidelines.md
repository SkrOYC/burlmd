...
TechSpec v2.1.10 (translated from reviewed v1.8.7-provisional) permits research code only under `.constitution/prototypes/`. The existing Epic G M0 production exceptions remain unchanged. `BURL-M015` and `BURL-M003` may write production code only for their reproducibility and validation bootstrap. Tasks v3.2.10 adopted the v2.1.9 receipt-integrity delta. Stage 4 must adapt `BURL-M003` and `BURL-O001` to the v2.1.10 Linux closure view before implementation continues. Every other production ticket remains blocked by its own decision evidence and a matching Stage 3 and Stage 4 adaptation. Except for these contract-scoped exceptions, production directories (`lib/`, `rust/`, `linux/`, and `macos/`) are read-only inputs to this research wave.
...
Linux profiles without Nix command authority receive no Nix database. The closure manifest and executable checks are sufficient because these profiles run realized paths only. `BURL-O001` needs a writable local store for offline build outputs. It requires Nix 2.35.2 from the pinned install action. Its trusted launcher exports metadata for exactly the manifest members with `nix-store --dump-db PATHS`, then loads that subset into a fresh private store. The namespace mounts the private state and empty output store as writable. It overlays every host closure member read-only at its canonical path. The candidate can register private derivations and outputs, but it can't write host closure bytes or query ambient host-store metadata.

Capacity checks use allocated filesystem bytes rather than Nix Archive (NAR) size. Immediately before build output, the launcher measures canonical, nonoverlapping roots on the containing filesystem. The inventory includes the host CI and devenv union. One workspace root includes both checkouts and their dependency caches. Separate roots cover sparse private state, the private output store, and every candidate root outside the workspace. The launcher rejects duplicate, nested, missing, or cross-filesystem roots. The sum must not exceed 10,000,000,000 bytes. It must leave at least 4,000,000,000 bytes against the standard runner's 14,000,000,000-byte profile.

For `BURL-O001`, the launcher writes a fresh capacity record immediately before and after each Linux package-build phase. The exact phase IDs are `linux-flake-check`, `linux-build`, `linux-flake-install`, `linux-appimage-installed-probe`, `linux-flake-installed-probe`, `ubuntu-22.04-x86_64`, `ubuntu-24.04-x86_64`, `ubuntu-26.04-x86_64`, `debian-12-x86_64`, and `debian-13-x86_64`. Each record identifies the phase, event, sequence, filesystem, allocated bytes, available bytes, thresholds, observation time, and output-root allocation. The trusted wrapper appends the records to `artifacts/linux-capacity.ndjson`. It includes the file in the sealed role bundle. Aggregation converts the complete ordered records into Spike-result measurements.

A phase can't start without its immediately preceding valid record. The launcher rejects the phase before output when available space is below 4,000,000,000 bytes. It stops the Spike with an insufficient-capacity result after any out-of-bound postcheck or storage-exhaustion failure. Fixtures invoke every named phase through a recording harness. They inject low space before a later phase and prove that the phase doesn't write output.

The standard `ubuntu-24.04` runner remains an assumed, fail-closed research choice. A local prototype reduced prebuild allocation from 15,382,421,504 bytes to 9,910,620,160 bytes. This result leaves a nominal 4,089,379,840 bytes, but the prototype didn't run the AppImage, Flake, distribution, or macOS archive sequence. It doesn't prove packaging feasibility.
