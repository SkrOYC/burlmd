...
2. **Dart:**
   ...
   - **Link-completion grammar is fixed.** A completion is eligible only when a collapsed caret has a last unmatched `[[` on the same line before it; its query is the intervening text. The UI snapshots that exact trigger range and source value, requests no more than 10 Core candidates, and rejects/dismisses the list if a newline, `]]`, focus/selection change, or edit invalidates the snapshot. Acceptance replaces the snapshot range with the Core-supplied `LinkCompletion.insert_text`; no Dart code may construct or repair a destination.
   - **A cross-Block selection uses one direct `TextInputClient` proxy, not a hidden `TextField`.** The proxy owns exactly one `TextInputConnection` while it owns the selection, forwards every complete editing value to one Core `replace_range` or `delete_range` call, then applies the returned `RangeEditResult` caret. It closes its connection before promotion to a focused Block and on dispose. While `TextEditingValue.composing` is valid and non-collapsed it must neither mutate Core nor replace the platform's composing value; composition resolves or is cancelled before dispatch. This preserves keyboard, clipboard and IME behavior without making Dart an owner of Note content (ADR-012).
3. **Nix:**
