...
Each signer is a reusable workflow with exactly one role job and a fixed hosted label. The caller uses the full candidate commit SHA as the reusable-workflow ref. The expected identity records the resulting `job_workflow_ref` and `job_workflow_sha`, and requires the SHA in both fields to equal the candidate head. The expected-identity digest covers the exact generated JSON bytes; roles receive that immutable file rather than reconstructing it.

Each role first writes bytes that validate against `contracts/ci-role-evidence.schema.json`. This pre-upload manifest contains the authoritative identity and exactly one role's hosted image, OS, architecture, CPU, memory, storage, logical viewport, toolchains, internal artifacts, and gate results. It doesn't contain an artifact ID, upload digest, attestation, or other value assigned after upload. The role computes the manifest's SHA-256 digest, attests those exact bytes, and uploads the unchanged manifest. This order removes a digest cycle.

The coordinator downloads the uploaded object, verifies the service-reported upload digest, extracts the one expected manifest, and verifies its byte digest against the attestation subject. The signed SLSA predicate's builder identity must equal the role's expected `job_workflow_ref`, including its distinct path and 40-hex SHA. Its invocation must match the expected run and attempt. The coordinator then uses the versioned job API to verify that the same run contains the signer workflow's single job with its fixed hosted label. It doesn't correlate artifact and job REST objects directly, and it doesn't trust an artifact-supplied job name or ID.

Aggregation runs either in a coordinator job or locally with authenticated read access. It compares each manifest with the independently supplied expected identity and writes `contracts/ci-evidence.schema.json`. An `accepted` result contains all three enriched roles, successful aggregation checks, and no rejection reasons. A `rejected` result can contain partial or no role evidence and must contain one or more typed rejection reasons. Therefore missing, mismatched, stale, corrupt, or unauthenticated evidence produces a durable fail-closed result instead of becoming schema-invalid.
...
Aggregation verifies each manifest's internal hashes and compares its captured identities with the independently supplied expected identity. It also requires the exact evidence-class sequence for each role. Linux can't claim macOS visual evidence, macOS can't claim Linux visual evidence, and macOS 15 can't claim performance or visual evidence. An operator commits the accepted or rejected aggregate report to the candidate branch; the workflow never pushes repository content. Milestone review starts only from an accepted evidence commit. A draft-run status alone is not evidence.
