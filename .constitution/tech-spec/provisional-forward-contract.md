...
Linux candidate execution uses the version `36` exact closure view. The trusted launcher mounts each enumerated host store path read-only at the same `/nix/store` path inside Bubblewrap. No host store root, database, or daemon socket enters the namespace. A read-only manifest replaces the closure environment list. It binds dependency completeness, manifest identity, the visible path set, `ARG_MAX` headroom, and executable loader checks. Each namespace disables nested user namespaces. `BURL-O001` alone loads the manifest's database subset into sparse private state and writes private Nix outputs.

The Linux capacity gate measures allocated bytes across the complete prebuild filesystem inventory. It rejects more than 10,000,000,000 allocated bytes or less than 4,000,000,000 bytes of headroom against the standard 14,000,000,000-byte profile. It does not use Nix Archive (NAR) size as a capacity proxy. The measured corrected prebuild allocation is 9,910,620,160 bytes, including sparse-store overhead under the workflow-pinned Nix 2.35.2.
