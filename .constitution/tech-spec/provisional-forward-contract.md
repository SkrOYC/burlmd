...
Base sessions mount no compositor member, expose no Wayland variable or socket, and run no compositor process. Only the two integration sessions start Sway and the candidate as untrusted siblings under the trusted in-namespace supervisor. Sway uses `env -i`, the headless backend, pixman renderer, Xwayland-disabled reviewed config, and exact launch arguments. It creates `/candidate/xdg/runtime/wayland-1` inside the namespace. The candidate receives that basename and runtime path, not a parent socket.

The supervisor terminates and reaps Sway on success, candidate failure, early Sway failure, timeout, socket disruption, and interruption. It verifies that the PID no longer exists. With no hostile process alive, it writes the cleanup record. The parent validates and fsyncs the retained session frame before acknowledging namespace exit. Candidate output, Sway, `swaymsg`, and their sockets remain untrusted. Only fresh sealing is authoritative.
