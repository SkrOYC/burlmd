...
ADR-020 and raw contract version `36` define one additional implementation detail for `BURL-M003`: an exact read-only Bubblewrap view of its complete locked runtime closure. A pre-namespace check requires a fresh staging root with only empty, source-distinct placeholders. The view hides the host store root, `/nix/var`, the Nix database, the daemon socket, and unlisted paths. A trusted preflight verifies every member's decoded mountinfo root, device, read-only option, file type, `st_dev`, and `st_ino` against a read-only host-source snapshot. An omitted bind or copied placeholder can't pass visible-name equality alone. Static source fixtures permit only the owned-store base, same-path manifest-member binds, and one `--symlink` from the mounted canonical store-backed `env` to `/usr/bin/env`. Every other bind-family option involving `/nix`, any bind at `/usr/bin/env`, and any broad `/usr` bind fails. A `#!/usr/bin/env bash` fixture proves the symlink. A fixture that hides copied state beneath valid member binds fails the staging-shape check before Bubblewrap starts.

The trusted parent proves the exact canonical Bubblewrap store path and exact `bubblewrap 0.11.2` output, then uses that same path for the sole candidate Bubblewrap invocation. The in-namespace preflight and candidate don't execute Bubblewrap. They assert only the resulting namespace, mount, process, descriptor, and network properties. The view preserves ADR-0019's Linux isolation controls and OD-11 authority.

Before the first candidate command, the launcher enumerates every BURL-M003 capacity-relevant writable root. This includes checkouts, caches, build and generated outputs, temporary and output roots, candidate state, closure staging, session storage, and private Sway runtime. It also includes each writable-bind source and writable environment path. The launcher groups the complete inventory by distinct `st_dev` and applies the 4,000,000,000-byte start guard to every distinct containing filesystem. An omitted root or device and a low-space secondary filesystem fail closed. This is a start guard, not a phase transcript, peak measurement, or `BURL-O001` packaging claim.

The Linux role retains `logs/burl-m003-linux-closure-view.log` through the unchanged `roleEvidence.internalArtifacts` fields. Raw contract version `36` defines the log's exact byte grammar, length-prefixed preflight record, escaping, tokens, literal manifest delimiter, final-byte rule, and canonical golden fixture. The log contains the exact Bubblewrap identity, manifest and SHA-256, per-member source observations, complete-vector byte count, `ARG_MAX`, root-to-device inventory, and per-device start-space observations. Fresh sealing validates those bytes and values. Accepted aggregation embeds the role manifest and binds the unchanged role and sealed bundle digests. This evidence path doesn't add a role-schema field, aggregate-schema field, or service artifact.

## Contract-scoped production authorization

The Epic G M0 production exceptions remain in force. `BURL-M015` and `BURL-M003` may also implement the reproducibility and managed-validation bootstrap in this TechSpec. Stage 4 must adapt `BURL-M003` to raw contract version `36`, including the `env` symlink, trusted-parent Bubblewrap proof, log grammar, and exhaustive per-device start guard. It must add or retain a `BURL-O001` stop condition that routes Linux Nix packaging isolation and capacity to a later Stage 3 decision. This patch doesn't authorize or solve that `BURL-O001` work. `BURL-M003` owns the missing `scripts/check-generated-bindings.sh` file and must create the non-mutating checker before any bootstrap gate invokes it. The checker snapshots and backs up both generated surfaces. It runs the provisional Flutter Rust Bridge `2.12.0` generator, compares file sets and bytes, and reports stale output. The checker restores the exact precheck state before exit. Implementation must follow the accepted contract without importing or cherry-picking the coordinating Epic G branch. This authorization doesn't settle any open AST, path, asset, Git, observer, packaging, or release choice.
