...
The release control surface treats `scripts/prepare-compatibility-stage.sh`, `scripts/record-compatibility-stage-rest.sh`, `scripts/write-compatibility-stage-lineage.sh`, and `scripts/prepare-compatibility-stage-consumer.sh` as immutable trust-anchor controls. The first three helpers create, bind, and record lineage for the macOS 26 seal-owned stage. The macOS 15 trusted workflow wrapper runs the three offline attestation-verification commands. The fourth helper consumes and validates their result files and the complete static inputs, validates lineage and freshness, removes credentials, and exposes only verified read-only members to the macOS 15 candidate. A byte or mode change in any helper requires reviewed trust-anchor rotation and evidence-only completion.
