...
ADR-020 and raw contract version `36` define one additional implementation detail for `BURL-M003`: an exact read-only Bubblewrap view of its complete locked runtime closure. The view hides the host store root, Nix state, database, daemon, and unlisted paths. It preserves the existing Linux isolation controls and prevents candidate-created user namespaces in the same Bubblewrap invocation. Its fixtures retain the complete-vector `ARG_MAX`, manifest, view, read-only, canary, loader, runtime, and forbidden-client checks.
