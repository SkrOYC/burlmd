...
ADR-020 defines the assumed version `36` Linux closure view. The trusted launcher mounts each enumerated host store path read-only at the same `/nix/store` path inside Bubblewrap. No host store root, database, or daemon socket enters the namespace. A read-only manifest replaces the closure environment list. It binds dependency completeness, manifest identity, the visible path set, `ARG_MAX` headroom, and executable loader checks. Each namespace disables nested user namespaces. `BURL-O001` alone loads the manifest's database subset into sparse private state and writes private Nix outputs.

The Linux capacity gate measures allocated bytes across the complete prebuild filesystem inventory. It rejects more than 10,000,000,000 allocated bytes or less than 4,000,000,000 bytes of headroom against the standard 14,000,000,000-byte profile. It doesn't use Nix Archive (NAR) size as a capacity proxy. A recording wrapper must recheck capacity before and after every named `BURL-O001` Linux build phase. Fixtures inject low space before a later phase and require rejection before that phase writes output.

The local corrected prebuild observation is 9,910,620,160 bytes, including sparse-store overhead under Nix 2.35.2. It leaves 4,089,379,840 bytes in the nominal profile. The prototype didn't run the complete AppImage, Flake, distribution, or macOS archive sequence. The standard `ubuntu-24.04` runner is therefore an assumed, fail-closed research choice, not a settled packaging-capacity claim. A managed `BURL-O001` run must record the packaging peak or stop with an insufficient-capacity result.

## Contract-scoped production authorization

The existing Epic G M0 production exceptions remain in force. `BURL-M015` and `BURL-M003` may also implement the reproducibility and managed-validation bootstrap in this TechSpec. Tasks v3.2.10 adopted version `35`. Stage 4 must adapt `BURL-M003` and `BURL-O001` to the version `36` Linux closure view before implementation continues. `BURL-M003` owns the missing `scripts/check-generated-bindings.sh` file and must create the non-mutating checker before any bootstrap gate invokes it. The checker snapshots and backs up both generated surfaces. It runs the provisional Flutter Rust Bridge `2.12.0` generator, compares file sets and bytes, and reports stale output. The checker restores the exact precheck state before exit. Implementation must follow the accepted contract without importing or cherry-picking the coordinating Epic G branch. This authorization doesn't settle any open AST, path, asset, Git, observer, packaging, or release choice.
