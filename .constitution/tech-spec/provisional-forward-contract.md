...
Every other production ticket remains blocked until its own decision evidence lands. Product Requirements and Architecture are reviewed when that evidence changes an upstream assumption. Final Stage 3 then replaces that ticket's provisional physical contract, and Stage 4 adapts the ticket before implementation. The existing Stage 4 Tasks artifacts for `BURL-M003` and `BURL-O001` are also downstream-stale for this interface split. A separate `planning-engineering-execution` adaptation must revise those Tasks artifacts and the graph as needed before implementation. This Stage 3 pass does not edit `.constitution/tasks` and makes no claim that Task scope is unchanged. A ticket doesn't wait for an unrelated Spike, and one completed Spike doesn't authorize neighboring production work.
