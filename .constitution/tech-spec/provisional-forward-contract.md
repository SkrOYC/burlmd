...
The release control surface treats `scripts/prepare-compatibility-stage.sh`, `scripts/record-compatibility-stage-rest.sh`, `scripts/write-compatibility-stage-lineage.sh`, `scripts/prepare-compatibility-stage-consumer.sh`, and `scripts/validate-compatibility-stage-interface.sh` as immutable trust-anchor controls. Static workflow-shape fixtures use duplicate-aware source parsing to pin the exact ordered 26 producer-output keys, consumer string-input keys, and observable trusted mappings. They also pin every canonical hyphenated consumer input through its uppercase environment variable into the validator's ordered 26-field inventory. Before caller mapping, the macOS 26 output boundary explicitly ticket-gates and conditionally normalizes the three producer sealing aliases to empty strings for every ticket other than `BURL-O001`; it does not rely on omitted reusable-workflow input defaults. Before acquisition or consumer processing, the macOS 15 trusted workflow wrapper invokes the fifth helper. Accepted nonempty `BURL-O001` values proceed to all three artifact downloads, offline verification, and the fourth helper. Accepted all-empty values for every other ticket skip only those compatibility steps, then continue the ordinary candidate path. The runtime shell cannot validate source key shape or arbitrary value provenance. The fourth helper validates the result files, and downstream attestation, identity, digest, lineage, and freshness checks validate the values it consumes. It then removes credentials and exposes only verified read-only members to the macOS 15 candidate. A byte or mode change in any helper requires reviewed trust-anchor rotation and evidence-only completion.
...
Every other production ticket remains blocked until its own decision evidence lands. Product Requirements and Architecture are reviewed when that evidence changes an upstream assumption. Final Stage 3 then replaces that ticket's provisional physical contract, and Stage 4 adapts the ticket before implementation. Tasks v3.2.6 adopted the complete interface for `BURL-M003` and `BURL-O001`: the ordered 26-field mapping, both macOS 26 output-boundary normalizations, three distinct offline-verification result consumptions, and `downloadedStageArtifactId` equality acceptance with mismatch rejection. That adoption closes the planning gate. The v3.2.6 changelog retains `upstream.tech-spec: v2.1.5`; a later Stage 4 metadata-only alignment must update the pointer to v2.1.6. This Stage 3 patch doesn't edit `.constitution/tasks`. A ticket doesn't wait for an unrelated Spike, and one completed Spike doesn't authorize neighboring production work.
