...
4. **Connecting is additive and later.** CAP-SYNC-01's connect flow authorizes a provider, provisions or selects a repository, adds it as a remote to the *existing* local repository, and pushes the history that already exists. It never re-homes, re-clones, or discards local state, and `workspaces.provider` transitions from `'local'` to the provider name in place.
   Physically, `BND-10` asks Provider (`BND-14`) only for authorization, eligible private-Remote selection or provisioning, and location. `BND-10` then transfers history to Remote (`BND-20`). Object transfer and storage remain `BND-11` and `BND-21` responsibilities.
...
- **Negative:** Three bootstrap paths now exist (initialize local, clone existing, adopt an existing directory) where the specification previously described one. All three must converge on the same post-conditions — index initialized, root key present, `workspaces` row written, repository present — or later code will need to ask which path it came from. The third is the reason decision 8 exists: it was originally described as initializing nothing, which made it the one path that did not converge.
