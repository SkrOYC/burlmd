...
2. Core requests a device code, shows GitHub's verification URI and user code, and polls the token endpoint at no less than the returned interval. It handles pending, `slow_down`, expiry, denial, and restart explicitly.
3. Expiring user access tokens and refresh tokens are stored only in Platform secure storage. Refresh is serialized and rotates the stored pair atomically. A rejected or expired refresh returns to device authorization without deleting local Workspace state.
4. The GitHub App requests repository Contents read/write for HTTP Git. Private-repository provisioning additionally requires Administration write; the connection UI explains that permission before authorization. Metadata read is implicit. No webhook permission is requested.
