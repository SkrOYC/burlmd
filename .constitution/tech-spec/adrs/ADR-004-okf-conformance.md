...
7. **Do not declare `okf_version` on disk.** §12 permits a bundle to declare its target version, but only via `okf_version` in the frontmatter of a bundle-root `index.md`. Since decision 6 declines to generate that file, the bundle ships untagged. The version this project targets is recorded here and in `stack.yaml` instead.
