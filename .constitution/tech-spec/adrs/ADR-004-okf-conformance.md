...
3. **Write two frontmatter fields, preserve everything else.** The application writes `type` (required by §11.2) and `title` (recommended by §4.1; note the specification does *not* require it, and permits consumers to derive a title from the filename — we write it anyway because it decouples display title from filename). Every other key found in an existing frontmatter block is preserved byte-for-byte and never reordered, reformatted, or dropped. See ADR-007 for the mechanism that makes this preservation free rather than engineered.
...
5. **Links are standard Markdown, bundle-absolute.** Per §6.1, absolute (bundle-relative, leading `/`) is the recommended form and is what is written. The `[[` sequence is retained purely as the UI trigger for a completion that inserts such a link; it is never stored. Recorded in `prd/out-of-scope/wikilink-syntax-on-disk.md`.
6. **Reserve `index.md` and `log.md`; never generate them.** Both are optional under §3.1/§8/§9, and conformance rule §11.3 constrains them only when present. Not generating them keeps the bundle conformant with no maintenance burden.
