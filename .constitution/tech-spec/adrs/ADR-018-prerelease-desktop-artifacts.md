...
Managed validation must run on the three roles in decision 8 and validate `contracts/ci-evidence.schema.json`. Linux exact visual proof extends the committed private headless Sway and Wayland capture. macOS 26 visual proof runs the actual hosted Apple Silicon GUI and owns one reviewed macOS baseline set. Both paths must prove a 1920x1080 at 60 Hz logical viewport. macOS 15 produces no visual baseline.

The Linux and macOS runs are aggregated only after GitHub Sigstore attestations and versioned Actions API metadata authenticate repository, workflow, job label, run, attempt, head revision, artifact ID, subject, and artifact digest. Aggregation compares captured identity with the independently supplied expected identity. These checks establish workflow origin; they don't make the weekly hosted image immutable or guarantee physical-host performance. Exact image and system facts remain mandatory evidence.
