...
8. The validation workflow uses OS-major-pinned standard hosted labels: `ubuntu-24.04`, Apple Silicon `macos-26`, and Apple Silicon `macos-15`. Every role runs the common functional matrix. Linux and macOS 26 run performance meters. macOS 26 alone owns the macOS visual baseline. macOS 15 is functional compatibility-only.
9. Release Pipeline creates an authoritative expected identity before role fan-out. It contains release, build, corpus, run, required-role, and required-role-signer identities. Each role uses a distinct single-job reusable workflow pinned to the candidate commit SHA. Per-role evidence captures the hosted image and system facts, authenticates its pipeline-owned origin, and travels through immutable artifacts with SHA-256 digests. Aggregation receives expected identity independently and rejects self-description, missing roles, origin failure, corruption, mismatch, duplication, and stale evidence.
10. Candidate validation runs on a candidate branch and draft pull request. Each role validates a pre-upload manifest that contains no service-assigned upload fields, then attests and uploads those unchanged bytes. A coordinator job or authenticated local process enriches the manifests with artifact and attestation facts and produces a discriminated accepted or rejected report. An operator can commit either report for evidence, but only an accepted report can start milestone review. The candidate workflow doesn't push repository content, publish a release, or receive live product credentials.
...
Managed validation must run on the three roles in decision 8. Role-produced bytes validate against `contracts/ci-role-evidence.schema.json`; the enriched coordinator report validates against `contracts/ci-evidence.schema.json`. Linux exact visual proof extends the committed private headless Sway and Wayland capture. macOS 26 visual proof runs the actual hosted Apple Silicon GUI and owns one reviewed macOS baseline set. Both paths must prove a 1920x1080 at 60 Hz logical viewport. macOS 15 produces no visual baseline.

The Linux and macOS runs are aggregated only after GitHub Sigstore attestations authenticate the exact role-specific reusable-workflow path and commit SHA, run, attempt, head revision, and manifest digest. The artifact API authenticates upload identity and digest. The job API confirms that the signer workflow's only job used its fixed hosted label. Aggregation doesn't attempt to correlate the artifact and job REST objects, and it doesn't trust a manifest-supplied job name or ID. It compares captured identity with the independently supplied expected identity. These checks establish workflow origin; they don't make the weekly hosted image immutable or guarantee physical-host performance. Exact image and system facts remain mandatory evidence.
