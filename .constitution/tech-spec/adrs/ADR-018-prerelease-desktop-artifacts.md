...
The Linux and macOS runs are aggregated only after GitHub Sigstore attestations authenticate the exact role-specific sealing workflow path, trusted default-branch signer SHA and ref, run, attempt, separate tested-source identity, and sealed role-bundle digest. The signed OIDC `runner_environment` claim must equal `github-hosted`. The aggregate retains distinct candidate and sealing job IDs, labels, environment facts, and artifacts. The versioned APIs must show the trusted static `candidate` → `seal` topology, successful jobs on the fixed hosted label, one unique candidate artifact, and one unique sealed artifact. The candidate upload's service ID and digest must match its trusted action outputs. Run-derived artifact names use only fixed prefixes, role, and the 32-hex suffix of `managed:<32hex>`. This correlation doesn't trust candidate-supplied job identity. Aggregation compares captured identity with independently supplied expected identity and verifies every manifest-named internal artifact against bundled bytes. These checks establish workflow origin; they don't make the weekly hosted image immutable or guarantee physical-host performance. Exact image and system facts remain mandatory evidence.
