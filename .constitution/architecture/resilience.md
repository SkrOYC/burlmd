...
- Local cache eviction requires a verified Object Store copy and no active offline need. burlmd doesn't delete authoritative Object Store bytes during `0.x` because history publication and Object Store deletion can't form one atomic transaction across the two external boundaries.
...
- Each validation role owns an isolated display, compositor, and input channel. The strict-containment role also proves candidate-process teardown. Other hosted roles record bounded cleanup without claiming arbitrary-process containment. A role that can't prove its assigned ownership or cleanup obligations emits no acceptable evidence.
- The Writer's active device never supplies visual proof or validation input. Captured Writer windows, desktop state, or active-device input invalidate the complete run.
- Release Pipeline establishes immutable trust-anchor identity from a reviewed validation implementation. It creates expected role, source, build, run, evidence-class, candidate-placement, topology, and completion identities outside the tested source. It gives the same expectation directly to validation and aggregation.
- Candidate commands are credential-free and receive no provenance authority. Trusted workflow fixes their placement and topology. Runtime observations confirm the expected label and terminal result. These observations are fail-closed guards, not cryptographic hosted-origin proof.
- A trusted wrapper can upload one complete candidate bundle, but that file handoff remains untrusted. The strict-containment role proves candidate-process teardown before upload. Other hosted roles perform bounded cleanup. A candidate survivor can corrupt or deny the upload and fail the role. It can't cross into a fresh environment or gain provenance authority.
- A fresh pipeline-owned sealing environment is the sole provenance and hosted-origin authority. It never executes candidate bytes. It validates the expected role and environment identity, exact handoff inventory, byte counts, and integrity identifiers. It authenticates an immutable sealed handoff only after those checks pass. Fresh-seal provenance alone cryptographically authenticates the sealing environment's hosted origin.
- For the cross-version compatibility handoff, the macOS 26 seal validates the producer members and creates the authenticated compatibility stage. It binds the stage's exact identifier and integrity digest to its seal locator and provenance. The trusted macOS 15 wrapper acquires that exact stage and verifies the binding. It removes the acquisition credential context before exposing the members read-only to the candidate. The candidate receives no credential or provenance authority. The macOS 15 seal validates the lineage against the trusted wrapper record and preserves it in sealed evidence.
- Evidence Aggregation observes sealing completion independently after the fresh environment finishes. It treats candidate placement and completion as runtime guards. It verifies the sealing environment's hosted origin only through fresh-seal provenance. For the cross-version compatibility handoff, it verifies the producer seal, stage identifier and integrity digest, macOS 15 consumer binding, and unchanged lineage. Any gap or substitution fails closed.
- Evidence Aggregation compares captured identity with authoritative expected identity and verifies the trust anchor and write boundary. It rejects premature completion, failed or missing sealing, candidate-defined validation control, duplicate handoffs, corrupt bundles, mismatches, and stale evidence. A later report state can't replace the tested-source identity.
...
- The validation implementation establishes trust through two reviewed integrations. The first lands the locally validated implementation and defines its immutable trust anchor. The merged implementation validates that exact source. A second evidence-only integration records the accepted result and completion state. Validation remains incomplete until both integrations merge. Any later change to trusted validation control repeats this bootstrap.
...
Release validation records isolation ownership, candidate placement and completion guards, sealing provenance, evidence handoffs, identity rejection, freshness rejection, credential-boundary checks, coordinator identity, and aggregation outcomes. Records for the cross-version compatibility handoff also correlate the producer seal, compatibility stage, and consumer binding. These records correlate a release identity without including the Writer's desktop content.
