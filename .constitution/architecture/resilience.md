...
- Release Pipeline establishes immutable trust-anchor identity from a reviewed validation implementation. It creates authoritative trust-anchor, validation-control signer, tested-source, base, release, build, corpus, run, required-role, and role-specific evidence-class identities outside the tested source, then hands the same expectation directly to validation and aggregation.
- Candidate commands receive no provenance authority. A trusted wrapper may upload one complete candidate bundle, but that file handoff remains untrusted. The strict-containment role proves candidate-process teardown before the wrapper uploads it; other hosted roles perform bounded cleanup. A candidate survivor can corrupt or deny that upload and thereby fail the role; it can't cross into a later fresh environment or gain provenance authority.
- A fresh pipeline-owned sealing environment is the sole provenance authority. It never executes candidate bytes. It validates expected role and environment identity, exact handoff inventory, byte counts, and integrity identifiers before authenticating an immutable sealed handoff.
- Evidence Aggregation observes sealing completion independently after the fresh environment finishes. It compares captured identity with authoritative expected identity, resolves the sealing locator to one completed successful role environment, and verifies the trust anchor and write boundary. It rejects a premature completion claim, an in-progress, failed, missing, or mismatched sealing phase, candidate-defined validation control, duplicate or substituted handoffs, corrupt bundles, mismatches, and stale evidence. A later report state can't replace the tested-source identity.
