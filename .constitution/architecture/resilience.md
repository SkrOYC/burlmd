...
- During replacement-store migration, a durable intent makes every publisher verify new Objects in both stores. Cutover uses compare-and-swap against the migration epoch, Workspace revision, and complete Remote-ref inventory after baseline and delta copy. Devices without replacement credentials pause publication but keep local use. Cutover retains the old store's non-secret fallback descriptor. A replacement miss can be repaired with securely supplied old-store credentials or by a credentialed peer; the old bytes are content-verified, backfilled and verified in the replacement, and only then hydrated.
