...
- Each validation run owns an isolated display, compositor, input channel, and declared process lifecycle. If it can't prove the required ownership, the run emits no acceptable evidence.
...
- Every validation role separates candidate execution from origin authentication. Candidate output is untrusted. The role with strict candidate containment must prove that its candidate process tree ends before upload. Other hosted roles record bounded cleanup and don't claim that arbitrary candidate processes ended. A fresh pipeline-owned sealing environment validates the bundle, authenticates immutable evidence, and records only identity and locator facts available before it finishes. Candidate processes cannot enter the fresh sealing environment or gain its origin authority.
