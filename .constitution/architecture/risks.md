...
- **Current implementation status (Epic B):** `save_note` implements OCC against the `notes.last_modified` column only — it compares `expected_base_revision` and rejects with a conflict error on drift, but no Markdown serializer or write-through to the on-disk file exists yet. This guards the database's own bookkeeping today, not the file-content-clobbering scenario described above, since no code path writes the AST back to disk yet and no Sync Manager exists to race it. Full protection is contingent on a future Markdown-serialization + Git-write-through ticket.
