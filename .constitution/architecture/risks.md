...
- **Risk:** Validation can capture the Writer's desktop, accept evidence from another environment, let candidate code corrupt its untrusted bundle, or expose aggregation credentials to candidate-controlled code.
...
- **Mitigation:** Release Pipeline runs from an immutable reviewed validation anchor and sends authoritative trust-anchor, workflow-signer, tested-source, base, build, corpus, run, and role identity directly to validation and aggregation. Before dispatch and during collection, it rejects a changed validation control surface or a tested-source delta outside the ticket boundary. Candidate output is untrusted and has no origin-signing authority. The strict-containment role proves teardown before upload. Other hosted roles use bounded cleanup. A surviving process can corrupt untrusted output or cause a failed upload, but can't enter the fresh sealing environment or gain origin authority. Aggregation authenticates that sealing origin and verifies the bundle without running candidate code. It then destroys remote credentials and runs the identified coordinator without network or ambient user state. It rejects unmanaged, candidate-controlled, out-of-boundary, missing, duplicated, unsealed, mismatched, stale, corrupt, or insufficiently isolated evidence. The later report state remains distinct from tested source.
