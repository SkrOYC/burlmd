...
# available to non-M003 tickets only; raw-38 M003 sessions use their separate
...
prepare_candidate_dependencies() {
  ...
  if [[ $role == linux-x86_64 ]]; then
    ...
    # package/plugin metadata while resolving. The raw-38 namespace must still
...
confirm_linux_namespace_teardown() {
  ...
  if [[ $ticket != BURL-M003 ]]; then
    ...
    # Bubblewrap's lock pathname. Raw-38 instead proves teardown through its
    # original retained descriptor below.
...
prepare_linux_candidate_private_store() {
  ...
  [[ $role == linux-x86_64 ]] || return 0
  [[ $ticket != BURL-M003 ]] || {
    echo 'BURL-M003 must use the raw-38 read-only closure views' >&2
    return 2
  }
...
prepare_linux_candidate_closure_views() {
  ...
  [[ -x $nix_executable && -x $nix_store_executable && $($nix_executable --version) == 'nix (Nix) 2.35.2' && $($nix_store_executable --version) == 'nix-store (Nix) 2.35.2' ]] || {
    echo 'raw-38 closure views require locked Nix 2.35.2' >&2; return 2;
  }
  # Resolve this once in the trusted parent, after dependency preparation and
  ...
  m003_revalidate_observed_executables || {
    echo 'trusted compositor or teardown executable does not match the raw-38 identity' >&2; return 2;
  }
  ...
  [[ $(wc -l <"$candidate_linux_base_manifest" | tr -d ' ') == 488 && $(wc -c <"$candidate_linux_base_manifest" | tr -d ' ') == 30717 && $(sha256sum "$candidate_linux_base_manifest" | awk '{print $1}') == 127043afe260d7756ee6cbda03e39a5bfceb4f7f79ae3a5be1595e447ce15e64 ]] || {
    echo 'base Linux closure view differs from the raw-38 locked manifest' >&2; return 2;
  }
  [[ $(wc -l <"$candidate_linux_integration_manifest" | tr -d ' ') == 547 && $(wc -c <"$candidate_linux_integration_manifest" | tr -d ' ') == 34315 && $(sha256sum "$candidate_linux_integration_manifest" | awk '{print $1}') == 353e927fb857fe8f8fc213da4ac53e0a12147ead779e2e29289c6a3034415896 ]] || {
    echo 'integration Linux closure view differs from the raw-38 locked manifest' >&2; return 2;
  }
  [[ $("$nix_executable" path-info --json "${candidate_linux_base_closure_paths[@]}" | jq -er '[.[] | .narSize] | add') == 6184635112 && $("$nix_executable" path-info --json "${candidate_linux_integration_closure_paths[@]}" | jq -er '[.[] | .narSize] | add') == 6338147240 ]] || {
    echo 'Linux closure views differ from the raw-38 NAR measurements' >&2; return 2;
  }
  ...
  [[ -x $bwrap_root/bin/bwrap && $("$bwrap_root/bin/bwrap" --version) == 'bubblewrap 0.11.2' && $(sha256sum "$bwrap_root/bin/bwrap" | awk '{print $1}') == c500b527e18f7e32634ac497b78a0150ceb31ae70fa8afef3fbbe79fd1d9f726 ]] || {
    echo 'trusted Bubblewrap does not match the raw-38 identity' >&2; return 2;
  }
  ...
  [[ $(wc -l <"$manifest_root/trusted-parent-bubblewrap.manifest" | tr -d ' ') == 8 && $(wc -c <"$manifest_root/trusted-parent-bubblewrap.manifest" | tr -d ' ') == 480 && $(sha256sum "$manifest_root/trusted-parent-bubblewrap.manifest" | awk '{print $1}') == 398d11c9cd9249369cbb18d36661014eafef5ac18ff7adeb076c2c51ef0141fd && $("$nix_executable" path-info --json $(<"$manifest_root/trusted-parent-bubblewrap.manifest") | jq -er '[.[] | .narSize] | add') == 40679016 ]] || {
    echo 'trusted Bubblewrap closure differs from the raw-38 inventory' >&2; return 2;
  }
...
# Raw contract version 38 deliberately keeps this in the trusted parent.  The
...
m003_openat2_current_identity() {
  ...
  perl -MFcntl=:DEFAULT -e '
    ...
    # Linux x86-64 raw-38 platform: openat2(2) is syscall 437. open_how is
...
m003_perl_session_controller() {
  ...
  perl -MFcntl=:DEFAULT -MIO::Handle -MErrno=EAGAIN -MFile::Copy=copy -MFile::Path=remove_tree -MPOSIX=dup2,WNOHANG -e '
    ...
    use constant SYS_inotify_add_watch => 254; # Linux x86_64 is the raw-38 platform.
...
linux_m003_candidate_bwrap() {
  ...
  shift
  [[ $ticket == BURL-M003 ]] || {
    echo 'raw-38 closure-view launcher received a non-M003 ticket' >&2
    return 2
  }
...
# ticket other than BURL-M003. Raw-38 sessions do not enter this function.
...
start_candidate_session() {
  ...
  if [[ $role == linux-x86_64 ]]; then
    ...
    if [[ $ticket == BURL-M003 ]]; then
      (umask 077; : >"$candidate_teardown_lock")
      exec {candidate_teardown_lock_fd}<>"$candidate_teardown_lock"
  ...
  (
    ...
    exec env -i "${candidate_env[@]}" "BURLMD_CANDIDATE_SESSION=$candidate_session" \
      ...
      "$candidate_tool_path/perl" -MPOSIX=setsid -e 'setsid() or die "setsid: $!"; exec @ARGV or die "exec: $!"' -- \
...
candidate_exec() {
  ...
  candidate_group_alive && { echo 'candidate session exceeded its bounded wait' >&2; terminate_candidate_session || true; return 1; }
...
run_integration_files() {
  local target=$1 runtime=$2 display=$3 failed=0 test_file result log json_log outcome step_id
  while IFS= read -r test_file; do
    ...
    set +e
    if [[ -n $runtime ]]; then
      candidate_exec env XDG_RUNTIME_DIR="$runtime" WAYLAND_DISPLAY="$display" GDK_BACKEND=wayland LIBGL_ALWAYS_SOFTWARE=1 flutter test --no-pub --suppress-analytics "$test_file" -d "$target" -r github --file-reporter="json:$json_log" >"$log" 2>&1
    else
      candidate_exec flutter test --no-pub --suppress-analytics "$test_file" -d "$target" -r github --file-reporter="json:$json_log" >"$log" 2>&1
    fi
    result=$?
    set -e
    if ((result == 0)) && integration_report_has_successful_tests "$json_log"; then status=passed; else status=failed; failed=1; fi
...
candidate_phase() {
  ...
  if [[ $ticket == BURL-M003 ]]; then
    ...
    if [[ $role == linux-x86_64 ]]; then
      # The raw-38 ordered session plan is the sole Linux candidate path.  It
      ...
      record_step generated-bindings . 'raw-38 managed base session' passed results/role-step-generated-bindings.log
      m003_run_session 2 flutter-test base flutter-test flutter test --no-pub --suppress-analytics
      record_step flutter-test . 'raw-38 managed base session' passed results/role-step-flutter-test.log
      m003_run_session 3 dart-analyze base dart-analyze dart --suppress-analytics analyze lib test integration_test test_driver
      record_step dart-analyze . 'raw-38 managed base session' passed results/role-step-dart-analyze.log
      ...
      record_step cargo-metadata . 'raw-38 managed base session' passed results/role-step-cargo-metadata.log
      ...
      if m003_consume_linux_integration_report 5 integration-378c340b182c219c9b1067d7918e48a70ed42307453d3b69402a1b075786b58d integration_test/production_host_flow_test.dart; then
        record_step integration-378c340b182c219c9b1067d7918e48a70ed42307453d3b69402a1b075786b58d . 'raw-38 managed integration session' passed results/role-step-integration-378c340b182c219c9b1067d7918e48a70ed42307453d3b69402a1b075786b58d.log
      else
        record_step integration-378c340b182c219c9b1067d7918e48a70ed42307453d3b69402a1b075786b58d . 'raw-38 managed integration session' failed results/role-step-integration-378c340b182c219c9b1067d7918e48a70ed42307453d3b69402a1b075786b58d.log
        return 1
      fi
      ...
      if m003_consume_linux_integration_report 6 integration-2aa91e4e2d4b0febdfc223ef1bae763bf139e2fc6f81409a05bf19c5c9139e9d integration_test/shell_flow_test.dart; then
        record_step integration-2aa91e4e2d4b0febdfc223ef1bae763bf139e2fc6f81409a05bf19c5c9139e9d . 'raw-38 managed integration session' passed results/role-step-integration-2aa91e4e2d4b0febdfc223ef1bae763bf139e2fc6f81409a05bf19c5c9139e9d.log
      else
        record_step integration-2aa91e4e2d4b0febdfc223ef1bae763bf139e2fc6f81409a05bf19c5c9139e9d . 'raw-38 managed integration session' failed results/role-step-integration-2aa91e4e2d4b0febdfc223ef1bae763bf139e2fc6f81409a05bf19c5c9139e9d.log
        return 1
      fi
      write_integration_outcomes_aggregate
      ...
      record_step managed-isolation . 'raw-38 managed base session' passed results/role-step-managed-isolation.log
      m003_finish_log
    else
