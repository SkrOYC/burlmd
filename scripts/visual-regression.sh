...
# This launcher must not let even its setup helpers observe a caller's desktop.
# Later commands explicitly pass the private Wayland and Sway state they need.
...
# smoke-shot builds the isolated release bundle before it can publish the app
# PID. Keep that build outside the short compositor-geometry race window; a
# cold pinned Rust cache can legitimately take longer than one minute; the
# Flutter/Linux plugin build may independently compile its native bridge.
CAPTURE_DEADLINE=$(( $(date +%s) + 900 ))
APP_PID=""
CLIENT_VERIFIED=0
