...
jq -e '
  ...
  . as $r |
  ($r | type == "object") and
  (($r | keys | sort) == ["artifactNonce", "baseSha", "candidateArtifact", "compatibilityStage", "expectedArtifact", "expectedIdentitySha256", "repositoryId", "role", "roleBundleSha256", "runAttempt", "runnerEnvironmentClaim", "schemaVersion", "sealedArtifact", "sealedBundleSha256", "sealingCheckRunId", "testedSourceSha", "ticketIdentity", "trustAnchorSha", "workflowPath", "workflowRunId", "workflowSignerRef", "workflowSignerSha"]) and
  ($r.schemaVersion == 2) and ($r.ticketIdentity | IN("BURL-M003", "BURL-H001", "BURL-H002", "BURL-I001", "BURL-L001", "BURL-O001", "BURL-G011", "BURL-P002", "BURL-O004", "BURL-O011", "BURL-O012", "BURL-O013")) and
  (if $r.ticketIdentity == "BURL-O001" and $r.role == "macos-26-arm64" then ($r.compatibilityStage | type == "object" and (keys|sort) == ["producerStage"])
   elif $r.ticketIdentity == "BURL-O001" and $r.role == "macos-15-arm64" then ($r.compatibilityStage | type == "object" and (keys|sort) == ["consumerBinding","producerLineage","producerLineageArtifact","producerLineageSha256"] and .consumerBinding.downloadedStageArtifactId == .producerLineage.stageArtifact.artifactId)
   else $r.compatibilityStage == null end) and ($r.expectedIdentitySha256 | sha) and
