...
sway_pid=
cleanup() {
  [[ -z $sway_pid ]] || { kill "$sway_pid" 2>/dev/null || true; wait "$sway_pid" 2>/dev/null || true; }
  # The private Nix store intentionally becomes read-only after its verified
  # copy.  Delete only this owned fixture root without following its store
  # symlinks or crossing a filesystem, restoring owner write bits first.
  [[ -d $scratch && $scratch == "${TMPDIR:-/tmp}"/burlmd-role-cold.* ]] || return 1
  find -P "$scratch" -xdev -type d -exec chmod u+rwx -- {} + 2>/dev/null || true
  find -P "$scratch" -xdev -type f -exec chmod u+rw -- {} + 2>/dev/null || true
...
mesa_egl=${BURLMD_MESA_EGL_VENDOR_PATH:?locked Mesa EGL vendor path is required}
[[ $mesa_dri == /nix/store/* && -d $mesa_dri && $mesa_egl == /nix/store/* && -f $mesa_egl ]]
openssl_entry=${openssl_pkgconfig#/nix/store/}
mapfile -t closure_paths < <(
  { printf '%s\n' "${closure_paths[@]}"; nix-store -qR "/nix/store/${openssl_entry%%/*}"; nix-store -qR "${openssl_include%/include}"; nix-store -qR "${openssl_lib%/lib}"; nix-store -qR "${mesa_dri%/lib/dri}"; nix-store -qR "${mesa_egl%/share/glvnd/egl_vendor.d/50_mesa.json}"; } | LC_ALL=C sort -u
)
...
private_root=$scratch/burlmd-private-nix.cold0001
mkdir -p "$private_root"
nix copy --offline --to "$private_root" --no-check-sigs "${closure_paths[@]}"
...
# are an overlapping read-only mount. The candidate sees only the writable
# private rooted Nix store copied for this M003 profile, never host /nix. The
# initial authenticated closure is overlaid read-only from that private store;
# only new private paths remain writable.
declare -a private_closure_binds=()
for logical_path in "${closure_paths[@]}"; do
  private_path=$(realpath -e -- "$private_root/nix/store/${logical_path#/nix/store/}")
  [[ $private_path == "$private_root"/nix/store/* && ! -L $private_path && "/nix${private_path#"$private_root/nix"}" == "$logical_path" ]] || {
    echo "invalid private closure member: $logical_path" >&2; exit 1;
  }
  private_closure_binds+=(--ro-bind "$private_path" "$logical_path")
done
private_env_interpreter=$(realpath -e -- "$private_root/nix/store/${env_interpreter#/nix/store/}")
[[ $private_env_interpreter == "$private_root"/nix/store/* && ! -L $private_env_interpreter && "/nix${private_env_interpreter#"$private_root/nix"}" == "$env_interpreter" ]]
env -i PATH="$path" HOME="$scratch/home" TMPDIR="$scratch/tmp" \
  ...
  --setenv PKG_CONFIG_PATH "$openssl_pkgconfig" --setenv LIBCLANG_PATH "$LIBCLANG_PATH" --setenv NIX_CFLAGS_COMPILE "-isystem $openssl_include" --setenv NIX_LDFLAGS_x86_64_unknown_linux_gnu "-L$openssl_lib" --setenv CFLAGS "-isystem $openssl_include" --setenv LDFLAGS "-L$openssl_lib" \
  --setenv RUSTFLAGS "-L native=$openssl_lib" \
  --setenv LIBGL_DRIVERS_PATH "$mesa_dri" --setenv __EGL_VENDOR_LIBRARY_FILENAMES "$mesa_egl" \
  --setenv BURLMD_LOCKED_NIX_CLOSURE "$locked_closure" --setenv BURLMD_PRIVATE_CLOSURE_SENTINEL "${closure_paths[0]}" \
  ...
  --setenv XDG_RUNTIME_DIR /candidate/xdg/runtime --setenv WAYLAND_DISPLAY "$sway_display" --setenv GDK_BACKEND wayland --setenv LIBGL_ALWAYS_SOFTWARE 1 \
  --setenv GIT_CONFIG_NOSYSTEM 1 --setenv GIT_CONFIG_GLOBAL /dev/null --setenv GIT_CONFIG_COUNT 0 --setenv GIT_TERMINAL_PROMPT 0 \
  --proc /proc --dev /dev --tmpfs /tmp --bind "$private_root/nix" /nix "${private_closure_binds[@]}" --dir /usr --dir /usr/bin \
  ...
  --dir /candidate/xdg/cache --dir /candidate/xdg/config --dir /candidate/xdg/data --dir /candidate/xdg/state --dir /candidate/xdg/runtime \
  ...
  --ro-bind "$sway_runtime/$sway_display" "/candidate/xdg/runtime/$sway_display" \
  ...
  --ro-bind "$private_env_interpreter" /usr/bin/env \
  ...
  --chdir /source "$bash_bin" -ceu '
    ...
    test ! -w "$BURLMD_PRIVATE_CLOSURE_SENTINEL"
    test ! -e /run/opengl-driver
    test ! -e /dev/dri
    printf cold-private-store-write > /nix/var/nix/cold-private-store-write
    ip link set lo up
    ...
    "$BURLMD_TRUSTED_BASH" /trusted/scripts/assert-managed-evidence-isolation.sh --contract /trusted/.constitution/tech-spec/contracts/provisional-spikes.toml --sandbox bubblewrap --expected-version 0.11.2
...
[[ $(<"$private_root/nix/var/nix/cold-private-store-write") == cold-private-store-write ]]
