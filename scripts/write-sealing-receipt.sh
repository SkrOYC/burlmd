...
stage=null
if [[ -n ${COMPATIBILITY_STAGE_FILE:-} ]]; then
  [[ -f $COMPATIBILITY_STAGE_FILE && ! -L $COMPATIBILITY_STAGE_FILE ]] || exit 1
  stage=$(jq -ce . "$COMPATIBILITY_STAGE_FILE")
fi
jq -cn --slurpfile expectedIdentity "$EXPECTED_IDENTITY_FILE" --argjson compatibilityStage "$stage" --arg expectedSha "$EXPECTED_IDENTITY_SHA256" --argjson repository "$GITHUB_REPOSITORY_ID" --argjson run "$GITHUB_RUN_ID" --argjson attempt "$GITHUB_RUN_ATTEMPT" --argjson check "$CHECK_RUN_ID" --arg role "$ROLE" --arg workflow "$WORKFLOW_PATH" --arg nonce "$ARTIFACT_NONCE" --arg tested "$TESTED_SOURCE_SHA" --arg base "$BASE_SHA" --arg trust "$trust_anchor" --arg expected "$EXPECTED_DIGEST" --arg candidateDigest "$CANDIDATE_DIGEST" --arg sealedDigest "$SEALED_DIGEST" --argjson expectedId "$EXPECTED_ID" --argjson candidateId "$CANDIDATE_ID" --argjson sealedId "$SEALED_ID" --arg roleHash "$(sha256sum "$candidate" | awk '{print $1}')" --arg sealedHash "$(sha256sum "$sealed" | awk '{print $1}')" '{schemaVersion:2,ticketIdentity:$expectedIdentity[0].ticketIdentity,expectedIdentitySha256:$expectedSha,repositoryId:$repository,workflowRunId:$run,runAttempt:$attempt,sealingCheckRunId:$check,workflowPath:$workflow,workflowSignerSha:env.GITHUB_SHA,workflowSignerRef:"refs/heads/master",trustAnchorSha:$trust,testedSourceSha:$tested,baseSha:$base,role:$role,artifactNonce:$nonce,runnerEnvironmentClaim:"github-hosted",expectedArtifact:{artifactId:$expectedId,artifactName:("managed-evidence-expected-"+$nonce),uploadActionDigest:$expected,artifactDigest:("sha256:"+$expected)},candidateArtifact:{artifactId:$candidateId,artifactName:("managed-evidence-candidate-"+$role+"-"+$nonce),uploadActionDigest:$candidateDigest,artifactDigest:("sha256:"+$candidateDigest)},sealedArtifact:{artifactId:$sealedId,artifactName:("managed-evidence-sealed-"+$role+"-"+$nonce),uploadActionDigest:$sealedDigest,artifactDigest:("sha256:"+$sealedDigest)},roleBundleSha256:$roleHash,sealedBundleSha256:$sealedHash,compatibilityStage:$compatibilityStage}' >"$output"
