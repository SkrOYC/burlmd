...
// Retained temporarily for the existing fixture-story helpers below; the gate
// mounts the coherent V2 shell state declared later in this file.
// ignore: unused_element
class _FixtureReferenceShellState extends State<FixtureReferenceShell> {
  var _selected = 'Focaccia';
  var _taskChecked = false;
  var _copied = false;
  String? _overlay;
  String _syncState = 'pendingSuggestions';
  String _searchScope = 'All';
  String _snapshot = '9a31f0e';

  void _open(String value) => setState(() => _overlay = value);
  void _close() => setState(() => _overlay = null);

  @override
  Widget build(BuildContext context) {
    final theme = buildBurlTheme(Brightness.dark);
    final c = BurlColors.dark;
    return Theme(
      data: theme,
      child: Material(
        ...
        child: Stack(
          ...
          children: [
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _FixtureShellControl(
                            key: const ValueKey('fixture-shell-delete'),
                            icon: LucideIcons.trash_2,
                            label: 'Delete Focaccia',
                            danger: true,
                            onTap: () => _open('delete'),
                          ),
                        ],
                  ...
                  child: Column(
                    children: [
                      Container(
                        ...
                        child: const Row(
                          children: [
                            _FixtureTab('Focaccia', selected: true),
                            _FixtureTab('Homelab'),
                            _FixtureTab('Recovered'),
                          ],
                        ),
                      ),
                      Container(
                      ...
                      Expanded(
                        child: Center(
                          child: ConstrainedBox(
                            ...
                            child: ListView(
                              key: const ValueKey('fixture-reference-content'),
                              ...
                              children: [
                                ...
                                if (_selected == 'Recovered')
                                  const Text(
                                    'Recovered draft',
                                    key: ValueKey(
                                      'fixture-reference-recovered-selected',
                                    ),
                                  ),
                                if (_selected == 'Homelab')
                                  const Text(
                                    'services:\n  paperless: true',
                                    key: ValueKey(
                                      'fixture-reference-homelab-selected',
                                    ),
                                    style: TextStyle(fontFamily: 'monospace'),
                                  ),
                                if (_selected != 'Focaccia')
                                  const SizedBox(height: 20),
                                const Text(
                                  'Focaccia',
                                  key: ValueKey('fixture-focaccia-h1'),
                                  style: TextStyle(
                                    fontSize: 30,
                                    height: 1.15,
                                ...
                                const SizedBox(height: 20),
                                const Text(
                                  'A slow, olive-oil-rich bread for crowded tables. Fold, rest, and let the dough make its own schedule.',
                                ...
                                const Text(
                                  'Mix the dough, let it rest, then press deep dimples into the surface. Finish with rosemary, salt, and a generous pour of olive oil.',
                                  style: TextStyle(fontSize: 15, height: 1.65),
                                ),
                                ...
                                Container(
                                  ...
                                  child: const Text(
                                    '“The oil is not optional. It is the point.”',
                                    style: TextStyle(
                                ...
                                const SizedBox(height: 18),
                                Container(
                                  key: const ValueKey('fixture-focaccia-code'),
                                ...
                                const SizedBox(height: 10),
                                const Text(
                                  'The oven runs hot. Reduce the final bake by two minutes.',
                                ),
                              ],
                            ),
                          ),
            ...
            if (_overlay != null) _buildOverlay(c),
          ],
        ),
      ),
    );
  ...
  Widget _buildOverlay(BurlColors c) => Positioned.fill(
    key: ValueKey('fixture-overlay-$_overlay'),
    child: Material(
  ...
  Widget _panel({
    ...
    child: BurlScaleFadeEntrance(
      duration: BurlMotion.overlay,
      child: Material(
        key: key,
        color: BurlColors.dark.editor,
        elevation: 18,
  ...
  Widget _preferences(BurlColors c) => _panel(
    ...
    child: SafeArea(
      child: SizedBox(
        ...
        child: Column(
          children: [
            _header(c, 'Editor Preferences', LucideIcons.settings),
            Expanded(
              child: ListView(
                key: const ValueKey('fixture-preferences-content'),
                padding: const EdgeInsets.fromLTRB(20, 20, 20, 24),
                children: [
                  _preferenceSection(c, 'Appearance Theme', [
                    'Washi Paper\nLight Mode',
                    'Sumi Ink\nDark Mode',
                  ], 1),
                  _preferenceSection(c, 'Base Reading Size', [
                    'Compact (14px)',
                    'Standard (16px)',
                    'Comfortable (18px)',
                    'Large (20px)',
                  ], 1),
                  _preferenceSection(c, 'Prose Line Measure', [
                    '55ch (Narrow reading)',
                    '65ch (Standard prose)',
                    '75ch (Wide / Technical)',
                    '85ch (Code & Tables)',
    ...
    child: Column(
      ...
      children: [
        ...
        GridView.count(
          ...
          mainAxisSpacing: 8,
          crossAxisSpacing: 8,
          childAspectRatio: columns == 3 ? 2.15 : 2.65,
          children: [
            for (var index = 0; index < options.length; index++)
              Container(
                key: ValueKey('fixture-preferences-$label-$index'),
                padding: const EdgeInsets.symmetric(
                  horizontal: 11,
                  vertical: 8,
                ),
                decoration: _box(c, selected: index == selected),
                child: Row(
                  children: [
                    Expanded(
                      child: Text(
  ...
  Widget _search(BurlColors c) => _panel(
    key: const ValueKey('fixture-search-palette'),
    alignment: const Alignment(0, -.518),
    width: 672,
    child: Column(
      ...
      children: [
        Container(
          ...
          child: Row(
            children: [
              Icon(LucideIcons.search, size: 17, color: c.accent),
              const SizedBox(width: 13),
              const Expanded(child: Text('sourdough')),
              Container(
        ...
        Container(
          ...
          child: Row(
            children: [
              Text('Scope:', style: _monospace(c)),
              const SizedBox(width: 10),
              for (final scope in ['All', 'Titles', 'Content'])
                Padding(
                  padding: const EdgeInsets.only(right: 5),
                  child: InkWell(
                    key: ValueKey('fixture-search-scope-$scope'),
                    ...
                    child: Container(
                      ...
                      decoration: _box(c, selected: _searchScope == scope),
                      child: Text(scope, style: _monospace(c)),
                    ),
                  ),
                ),
              const Spacer(),
  ...
  Widget _searchResult(
    ...
    padding: const EdgeInsets.all(8),
    decoration: BoxDecoration(
      color: selected ? c.focusedBlock : Colors.transparent,
  ...
  Widget _sync(BurlColors c) {
    final states =
        <String, ({String label, String description, IconData icon})>{
          'localOnly': (
            label: 'Local Only',
            description: 'Local files on disk without remote origin.',
            icon: LucideIcons.hard_drive,
          ),
          'connectedIdle': (
            label: 'In Sync',
            description: 'All notes match the remote Git repository.',
            icon: LucideIcons.check,
          ),
          'syncing': (
            label: 'Syncing',
            description: 'Synchronizing with remote repository.',
            icon: LucideIcons.refresh_cw,
          ),
          'offline': (
            label: 'Offline',
            description: 'Working locally; will sync when reconnected.',
            icon: LucideIcons.wifi_off,
          ),
          'pendingSuggestions': (
            label: '1 Pending Suggestion',
            description: 'Remote change ready for in-line block review.',
            icon: LucideIcons.git_pull_request,
          ),
          'authRequired': (
            label: 'Auth Required',
            description: 'SSH key or token required.',
          ...
          'externalChanged': (
            ...
            description: 'Files on disk updated externally.',
    ...
    return _panel(
      ...
      alignment: const Alignment(0, .036),
      width: 384,
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          _header(
            ...
            'Sync & Storage',
            null,
            dot: _syncState == 'connectedIdle'
                ? c.accent
                : _syncState == 'pendingSuggestions'
                ? c.review
                : c.textMuted,
          ),
          Padding(
              ...
              children: [
                Container(
                  ...
                  child: Row(
                    children: [
                      Icon(
                        state.icon,
                        key: _syncState == 'syncing'
                            ? const ValueKey('fixture-sync-spinner')
                            : null,
                        size: 15,
                        color: c.accent,
                      ),
                ...
                Container(
                  ...
                  child: Text(
                    ...
                    style: _monospace(c, height: 1.7),
                  ),
                ),
                const SizedBox(height: 15),
                Text('SIMULATION STATE', style: _caption(c)),
                const SizedBox(height: 5),
                DropdownButtonFormField<String>(
                  key: const ValueKey('fixture-sync-state-picker'),
                  initialValue: _syncState,
                  isDense: true,
                  ...
                  onChanged: (value) => setState(() => _syncState = value!),
                ),
              ],
            ),
          ),
  ...
  Widget _history(BurlColors c) => _panel(
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 10),
                          for (final version in _snapshots)
                            _snapshotCard(
                              c,
                              version.$1,
                              version.$2,
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          Container(
                            key: const ValueKey(
                              'fixture-history-snapshot-details',
                            ),
                            padding: const EdgeInsets.all(12),
                            decoration: _box(c),
                            child: Text(
                          ...
                          OutlinedButton.icon(
                            ...
                            label: Text('Restore Note to Snapshot $_snapshot'),
                          ),
                        ],
                      ),
                    ),
  ...
  Widget _snapshotCard(BurlColors c, String hash, String when, String title) =>
      InkWell(
        key: ValueKey('fixture-history-snapshot-$hash'),
        onTap: () => setState(() => _snapshot = hash),
        child: Container(
          margin: const EdgeInsets.only(bottom: 8),
          padding: const EdgeInsets.all(10),
          decoration: _box(c, selected: _snapshot == hash),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Row(
              ...
              Text(title, style: const TextStyle(fontSize: 12, height: 1.2)),
  ...
  Widget _delete(BurlColors c) => _panel(
        ...
        Padding(
          ...
          child: Container(
            ...
            child: Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Icon(LucideIcons.shield_check, size: 15, color: c.accent),
                const SizedBox(width: 9),
                Expanded(
                  child: Text(
  ...
  Widget _header(
    ...
    child: Row(
      children: [
        ...
        IconButton(
          key: ValueKey('fixture-$_overlay-close'),
          onPressed: _close,
            ...
            : FilledButton(
                ...
                style: FilledButton.styleFrom(
                  ...
                  backgroundColor: action == 'Delete'
                      ...
                      : c.textPrimary,
                  foregroundColor: c.editor,
                ),
                child: action == 'Delete'
                    ? const Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          Icon(LucideIcons.trash_2, size: 14),
                          SizedBox(width: 6),
                          Text(
                            'Delete',
  ...
  BoxDecoration _box(BurlColors c, {bool selected = false}) => BoxDecoration(
    color: selected ? c.focusedBlock : c.sidebar,
      ...
      child: Row(
        children: [
          Icon(
            icon,
            size: 14,
            color: danger ? BurlColors.dark.syncError : null,
          ),
          const SizedBox(width: 8),
          Expanded(
            child: Text(
...
class _FixtureNavRow extends StatelessWidget {
  const _FixtureNavRow(this.label, this.icon, {this.selected = false});
  final String label;
  final IconData icon;
  final bool selected;
  @override
  Widget build(BuildContext context) => Container(
    key: ValueKey('fixture-nav-$label'),
    margin: const EdgeInsets.only(bottom: 3),
    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 7),
    decoration: BoxDecoration(
      color: selected ? BurlColors.dark.hover : Colors.transparent,
      borderRadius: BorderRadius.circular(4),
    ),
    child: Row(
      children: [
...
class _FixtureTab extends StatelessWidget {
  const _FixtureTab(this.label, {this.selected = false});
  final String label;
  final bool selected;
  @override
  Widget build(BuildContext context) => Container(
    key: ValueKey('fixture-tab-$label'),
    width: 224,
    alignment: Alignment.centerLeft,
    ...
    decoration: BoxDecoration(
      border: Border(
        top: BorderSide(
          color: selected ? BurlColors.dark.accent : Colors.transparent,
          width: 2,
        ),
      ),
    ),
